...
TEST(BufferLeaseTest, RejectsZeroCapacity) {
  iree_async_buffer_lease_t lease;
  memset(&lease, 0xCD, sizeof(lease));
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_INVALID_ARGUMENT,
      iree_net_buffer_lease_allocate(0, iree_allocator_system(), &lease));
  EXPECT_EQ(lease.span.region, nullptr);
  EXPECT_EQ(lease.span.offset, 0u);
  EXPECT_EQ(lease.span.length, 0u);
  EXPECT_EQ(lease.release.fn, nullptr);
...
TEST(BufferLeaseTest, AllocationSizeOverflowClearsOutput) {
  iree_async_buffer_lease_t lease;
  memset(&lease, 0xCD, sizeof(lease));
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_OUT_OF_RANGE,
      iree_net_buffer_lease_allocate(IREE_HOST_SIZE_MAX,
                                     iree_allocator_system(), &lease));
  EXPECT_EQ(lease.span.region, nullptr);
  EXPECT_EQ(lease.span.offset, 0u);
  EXPECT_EQ(lease.span.length, 0u);
  EXPECT_EQ(lease.release.fn, nullptr);

...
iree_status_t MakeBindAddress(std::string* out_address) {
  *out_address = "cts-loopback-listener";
  return iree_ok_status();
}

iree_status_t MakeUnreachableAddress(std::string* out_address) {
  *out_address = "cts-loopback-unreachable";
  return iree_ok_status();
}
...
const TransportBackend& GetTransportBackend() {
  static const TransportBackend backend = {
      ...
      /*.make_unreachable_address=*/MakeUnreachableAddress,
  };
  return backend;
}

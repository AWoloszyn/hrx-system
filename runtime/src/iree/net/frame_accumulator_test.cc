...
static iree_status_t TestOnFrameComplete(void* user_data,
  ...
  received.has_movable_lease = lease != nullptr;
  ctx->frames.push_back(std::move(received));
  if (ctx->should_retain_lease && lease) {
    ctx->retained_lease = *lease;
...
static iree_host_size_t CalculateStorageSize(iree_host_size_t max_frame_size) {
...
class AccumulatorWrapper {
 public:
  AccumulatorWrapper(iree_host_size_t max_frame_size, TestContext* ctx)
      : storage_(CalculateStorageSize(max_frame_size)) {
    accumulator_ =
...
TEST(FrameAccumulatorTest, InitializeValidatesConfiguration) {
  alignas(iree_net_frame_accumulator_t)
      uint8_t storage[sizeof(iree_net_frame_accumulator_t) + 1];
  auto* accumulator = reinterpret_cast<iree_net_frame_accumulator_t*>(storage);
  iree_net_frame_length_callback_t frame_length = {TestFrameLength, nullptr};
  iree_net_frame_complete_callback_t frame_complete = {TestOnFrameComplete,
                                                       nullptr};

  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        iree_net_frame_accumulator_initialize(
                            accumulator, 0, frame_length, frame_complete));
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_INVALID_ARGUMENT,
      iree_net_frame_accumulator_initialize(accumulator, 1, {nullptr, nullptr},
                                            frame_complete));
  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        iree_net_frame_accumulator_initialize(
                            accumulator, 1, frame_length, {nullptr, nullptr}));
}
...
TEST(FrameAccumulatorTest, SingleCompleteFrameIsZeroCopy) {
  ...
  EXPECT_TRUE(ctx.frames[0].has_movable_lease);
  EXPECT_EQ(lease.release_count, 1);
}

TEST(FrameAccumulatorTest, OnlyFinalPackedFrameMayMoveLease) {
  TestContext ctx;
  constexpr iree_host_size_t kMaxFrameSize = 1024;
  AccumulatorWrapper wrapper(kMaxFrameSize, &ctx);
  ...
  EXPECT_FALSE(ctx.frames[0].has_movable_lease);
  EXPECT_FALSE(ctx.frames[1].has_movable_lease);
  EXPECT_TRUE(ctx.frames[2].has_movable_lease);
  EXPECT_EQ(lease.release_count, 1);
}

TEST(FrameAccumulatorTest, FinalPackedFrameCanMoveLease) {
  TestContext ctx;
  ctx.should_retain_lease = true;
  constexpr iree_host_size_t kMaxFrameSize = 1024;
  AccumulatorWrapper wrapper(kMaxFrameSize, &ctx);
  ...
  IREE_ASSERT_OK(iree_net_frame_accumulator_push_lease(
      wrapper.get(), &lease.lease, frame1.size() + frame2.size()));

  ASSERT_EQ(ctx.frames.size(), 2u);
  EXPECT_FALSE(ctx.frames[0].has_movable_lease);
  EXPECT_TRUE(ctx.frames[1].has_movable_lease);
  EXPECT_EQ(lease.release_count, 0);

  iree_async_buffer_lease_release(&ctx.retained_lease);
  EXPECT_EQ(lease.release_count, 1);
}
...
static iree_status_t NeverKnowsFrameLength(void* user_data,
                                           iree_const_byte_span_t available,
                                           iree_host_size_t* out_frame_size) {
  (void)user_data;
  (void)available;
...
TEST(FrameAccumulatorTest, BufferFullButCannotDetermineFrameSize) {
  ...
  std::vector<uint8_t> storage(CalculateStorageSize(kMaxFrameSize));
...
TEST(FrameAccumulatorTest, ExactFullBufferRequiresResolvedFrameSize) {
  TestContext ctx;
  constexpr iree_host_size_t kMaxFrameSize = 8;
  std::vector<uint8_t> storage(CalculateStorageSize(kMaxFrameSize));
  auto* accumulator =
      reinterpret_cast<iree_net_frame_accumulator_t*>(storage.data());
  IREE_ASSERT_OK(iree_net_frame_accumulator_initialize(
      accumulator, kMaxFrameSize, {NeverKnowsFrameLength, nullptr},
      {TestOnFrameComplete, &ctx}));

  MockLease lease(std::vector<uint8_t>(kMaxFrameSize, 0x42));
  IREE_EXPECT_STATUS_IS(IREE_STATUS_RESOURCE_EXHAUSTED,
                        iree_net_frame_accumulator_push_lease(
                            accumulator, &lease.lease, kMaxFrameSize));

  EXPECT_EQ(lease.release_count, 1);
  EXPECT_TRUE(ctx.frames.empty());
  iree_net_frame_accumulator_deinitialize(accumulator);
}

TEST(FrameAccumulatorTest, MalformedFrameHeaderPropagates) {
  TestContext ctx;
  constexpr iree_host_size_t kMaxFrameSize = 1024;
  AccumulatorWrapper wrapper(kMaxFrameSize, &ctx);

  std::vector<uint8_t> malformed_header = {2, 0, 0, 0};
  MockLease lease(malformed_header);
  IREE_EXPECT_STATUS_IS(
...
TEST(FrameAccumulatorTest, ValidBytesCannotExceedLeaseSpan) {
  TestContext ctx;
  AccumulatorWrapper wrapper(/*max_frame_size=*/1024, &ctx);
  MockLease lease(std::vector<uint8_t>(4));

  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_OUT_OF_RANGE,
      iree_net_frame_accumulator_push_lease(wrapper.get(), &lease.lease, 5));

  EXPECT_EQ(lease.release_count, 1);
  EXPECT_TRUE(ctx.frames.empty());
}

TEST(FrameAccumulatorTest, NonEmptyLeaseRequiresStorage) {
  TestContext ctx;
  AccumulatorWrapper wrapper(/*max_frame_size=*/1024, &ctx);
...
TEST(FrameAccumulatorTest, StorageSizeCalculation) {
  ...
  IREE_ASSERT_OK(iree_net_frame_accumulator_calculate_storage_size(
      kMaxFrameSize, &storage_size));
  EXPECT_EQ(storage_size, expected_size);
}

TEST(FrameAccumulatorTest, StorageSizeRejectsOverflow) {
  iree_host_size_t storage_size = 0;
...
TEST(FrameAccumulatorTest, BufferedToZeroCopyTransition) {
  ...
  // Frame 2 is parsed directly but remains borrowed so frame 3 can be read.
  EXPECT_EQ(ctx.frames[1].data, frame2);
  EXPECT_FALSE(ctx.frames[1].has_movable_lease);
  // Frame 3 consumes the remaining receive bytes and may move the lease.
  EXPECT_EQ(ctx.frames[2].data, frame3);
  EXPECT_TRUE(ctx.frames[2].has_movable_lease);

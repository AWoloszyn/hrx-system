...
//
// Applications register transport factories during setup and look them up by
// scheme rather than hardcoding transport implementations:
//
...
// Register, lookup, enumerate, and count operations are thread-safe. Freeing
// the registry requires external synchronization with all other operations and
// borrowed factory uses.
...
// Registers a factory for the given scheme (for example, "tcp" or "loopback").
//
...
typedef iree_status_t(IREE_API_PTR* iree_net_transport_registry_enumerate_fn_t)(
...
// a non-OK status, iteration stops and that status is returned. Enumeration
// snapshots the current entries before invoking callbacks, so callbacks may
// query or register factories; newly registered factories are not visited by

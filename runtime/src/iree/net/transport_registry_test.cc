...
static iree_status_t mock_factory_create(iree_allocator_t allocator,
  ...
  iree_net_transport_factory_initialize(&mock_factory_vtable, &factory->base);
...
struct ReentrantEnumerateContext {
  iree_net_transport_registry_t* registry;
  iree_host_size_t observed_count = 0;
};

static iree_status_t enumerate_query_registry_callback(
    void* user_data, iree_string_view_t scheme,
    iree_net_transport_factory_t* factory) {
  auto* context = static_cast<ReentrantEnumerateContext*>(user_data);
  context->observed_count =
      iree_net_transport_registry_count(context->registry);
  return iree_ok_status();
}

TEST_F(TransportRegistryTest, EnumerateCallbacksRunOutsideRegistryLock) {
  mock_factory_t* factory = nullptr;
  IREE_ASSERT_OK(mock_factory_create(iree_allocator_system(), &factory));
  IREE_ASSERT_OK(iree_net_transport_registry_register(
...
TEST_F(TransportRegistryTest, FreeReleasesFactories) {
  ...
  iree_net_transport_factory_initialize(&tracking_vtable, &factory->base);

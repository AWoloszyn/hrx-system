...
static const char kIreeRunReplayUsage[] =
    ...
    "and barrier dependencies. Select the target topology with --device=.\n"
    "\n"
    "Usage:\n"
    ...
    "\n"
    "Examples:\n"
    "  iree-run-replay --device=task /tmp/model.ireereplay\n"
    "  iree-run-replay --device=task \\\n"
...
static void iree_run_replay_print_agent_markdown(FILE* file) {
  fputs(
      ...
      "## Capture\n"
      "\n"
      "Applications create an `iree_hal_replay_recorder_t` and wrap their\n"
      "device group with `iree_hal_replay_wrap_device_group`. Named scope\n"
      "markers can bracket initialization, execution, and teardown when\n"
      "the host needs those phases preserved in the capture.\n"
      "\n"
      ...
      "\n"
      "```bash\n"
      "iree-benchmark-replay --device=task --benchmark_min_time=20x \\\n"
      "  /tmp/model.ireereplay\n"
      "iree-benchmark-replay --device=task --benchmark_min_time=20x \\\n"

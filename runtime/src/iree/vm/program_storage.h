...
// One function field mapped between source order and its physical bank.
...
// nonzero token directly indexes this table without a provider query or scan.
...
typedef struct iree_vm_program_initializer_t {
  ...
  // Direct canonical callable ABI, or null without an initializer.
...
struct iree_vm_program_t {
  ...
  // Flat mapping words for module-local callable declarations.
...
static_assert(sizeof(void*) != 8 || sizeof(iree_vm_linked_module_t) <= 24,
              "64-bit linked modules must fit in 24 bytes");
static_assert(sizeof(void*) != 4 || sizeof(iree_vm_linked_module_t) <= 16,
              "32-bit linked modules must fit in 16 bytes");
static_assert(sizeof(void*) != 8 ||
                  sizeof(iree_vm_program_scalar_field_abi_t) <= 16,
              "64-bit scalar field ABIs must fit in 16 bytes");
static_assert(sizeof(void*) != 8 ||
...
static_assert(sizeof(void*) != 8 || sizeof(iree_vm_program_t) <= 128,
              "64-bit program headers must fit in 128 bytes");
...
// mapping. The check performs no provider call, name lookup, allocation, or
// ownership operation.
static inline bool iree_vm_program_function_ref_matches_mapping(
  ...
  if (!iree_vm_program_callable_token_is_valid(program, target_token) ||
      target_token != iree_vm_program_callable_token(expected_mapping)) {
    return false;
  }

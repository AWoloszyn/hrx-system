...
// Fuzz target for NFKD (Unicode Normalization Form KD) normalizer.
//
// Tests the NFKD normalizer's robustness against:
...
// - Compatibility decompositions (ligatures, fractions, fullwidth chars)
// - Long decompositions (U+FDFA decomposes to 18 codepoints)
// - Edge cases in canonical decomposition
//
...
// Process fuzz input through NFKD normalizer with streaming chunks.
static void process_with_chunk_size(iree_tokenizer_normalizer_t* normalizer,
  ...
  // NFKD can expand significantly (U+FDFA -> 18 codepoints).
  size_t output_capacity = size * 8 + 128;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  // Allocate NFKD normalizer.
  iree_tokenizer_normalizer_t* normalizer = NULL;
  iree_status_t status = iree_tokenizer_normalizer_nfkd_allocate(
  ...
  // Small chunks: stress combining sequence buffering and long decompositions.

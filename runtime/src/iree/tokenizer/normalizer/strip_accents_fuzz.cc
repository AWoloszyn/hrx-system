...
// Fuzz target for StripAccents normalizer.
//
// Tests the strip_accents normalizer's robustness against:
// - Unicode combining marks (Mn, Mc, Me categories)
// - Long combining character sequences
// - Mixed combining and non-combining characters
// - Invalid UTF-8 sequences
// - Precomposed characters (should pass through unchanged)
// - Indic scripts with spacing combining marks
// - Streaming with various chunk sizes
//
// This normalizer does not perform NFD decomposition. It only filters
// out characters in the Unicode Mark category. To strip accents from
// precomposed characters, chain NFD -> StripAccents.
//
...
// Process fuzz input through strip_accents normalizer with streaming chunks.
static void process_with_chunk_size(iree_tokenizer_normalizer_t* normalizer,
  ...
  // Output buffer sized same as input (strip_accents only removes bytes).
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  iree_status_t status = iree_tokenizer_normalizer_strip_accents_allocate(
  ...
  process_with_chunk_size(normalizer, data, size, 5);     // Prime size.

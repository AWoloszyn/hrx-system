...
// Fuzz target for ByteLevel decoder.
//
// Tests the byte-to-character mapping reversal against:
// - Shifted codepoints (U+0100-U+0143) that map to control chars, space, DEL
// - Identity codepoints (0x21-0x7E, 0xA1-0xAC, 0xAE-0xFF)
// - Passthrough codepoints (emoji, CJK, etc.)
// - Invalid UTF-8 sequences in token text
// - Mapped bytes that form invalid UTF-8 when decoded
// - Multi-token sequences with pending state across tokens
...
extern "C" int LLVMFuzzerInitialize(int* argc, char*** argv) {
  (void)argc;
  (void)argv;
  iree_status_t status = iree_tokenizer_decoder_byte_level_allocate(
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
...

  {
    ...
    for (int iter = 0; iter < 1000 && total_consumed < actual_token_count;
      ...
      if (strings_consumed == 0 && bytes_written == 0 && remaining.count > 0) {
        // Should make progress on next iteration with fresh buffer.
        continue;
      }
    }
  ...
  //===--------------------------------------------------------------------===//
  // Test 4: Zero capacity output (edge case)
  //===--------------------------------------------------------------------===//

  {
    ...
    if (iree_status_is_ok(status)) {
      iree_tokenizer_string_list_t token_list = {actual_token_count, tokens};
      iree_mutable_string_view_t zero_output = {output_buffer, 0};

      iree_host_size_t strings_consumed = 0;
      iree_host_size_t bytes_written = 0;
      status = iree_tokenizer_decoder_state_process(
          state, token_list, zero_output, &strings_consumed, &bytes_written);
      iree_status_ignore(status);

...
TEST(VocabBuilderTest, AddTokenAttrsNotFoundError) {
  ...
  IREE_EXPECT_STATUS_IS(IREE_STATUS_NOT_FOUND,
                        iree_tokenizer_vocab_builder_add_token_attrs(
...
TEST(VocabBuilderTest, EnsureTokenInsertsNewToken) {
  // ensure_token should insert when token doesn't exist.
  iree_tokenizer_vocab_builder_t* builder = nullptr;
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_allocate(
      10, iree_allocator_system(), &builder));

  // Insert token that doesn't exist.
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_ensure_token(
      builder, 5, IREE_SVL("<special>"), 0.0f,
      IREE_TOKENIZER_TOKEN_ATTR_SPECIAL));

  iree_tokenizer_vocab_t* vocab = nullptr;
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_build(builder, &vocab));

  // Token should exist at ID 5 (sparse vocab with gap at 0-4).
  EXPECT_EQ(iree_tokenizer_vocab_capacity(vocab), 6u);     // Array size: 0-5
  EXPECT_EQ(iree_tokenizer_vocab_token_count(vocab), 1u);  // Active tokens
  iree_string_view_t text = iree_tokenizer_vocab_token_text(vocab, 5);
  EXPECT_TRUE(iree_string_view_equal(text, IREE_SVL("<special>")));
  EXPECT_TRUE(iree_tokenizer_vocab_token_attrs(vocab, 5) &
              IREE_TOKENIZER_TOKEN_ATTR_SPECIAL);
  // Verify gaps are empty.
  EXPECT_EQ(iree_tokenizer_vocab_token_text(vocab, 0).size, 0u);
  EXPECT_EQ(iree_tokenizer_vocab_token_text(vocab, 4).size, 0u);

  iree_tokenizer_vocab_free(vocab);
}

TEST(VocabBuilderTest, EnsureTokenUpdatesExistingToken) {
  // ensure_token should update attrs when token already exists.
  iree_tokenizer_vocab_builder_t* builder = nullptr;
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_allocate(
      10, iree_allocator_system(), &builder));
  ...
  // Add a token first.
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_add_token_with_id(
      builder, 3, IREE_SVL("hello"), 0.0f, IREE_TOKENIZER_TOKEN_ATTR_NONE));

  // ensure_token with same ID should just add attributes.
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_ensure_token(
      builder, 3, IREE_SVL("ignored"), 0.0f,
      IREE_TOKENIZER_TOKEN_ATTR_SPECIAL));

  iree_tokenizer_vocab_t* vocab = nullptr;
  IREE_ASSERT_OK(iree_tokenizer_vocab_builder_build(builder, &vocab));

  // Token should have original text but new attributes.
  iree_string_view_t text = iree_tokenizer_vocab_token_text(vocab, 3);
  EXPECT_TRUE(iree_string_view_equal(text, IREE_SVL("hello")));

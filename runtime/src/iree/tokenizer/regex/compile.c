  ...
  *out_dfa_size = 0;
  ...
  if (pattern.size == 0) {
    ...
    IREE_RETURN_AND_END_ZONE(
        z0, iree_make_status(IREE_STATUS_INVALID_ARGUMENT, "empty pattern"));
  }
  ...
  iree_status_t status =
      iree_tokenizer_regex_parse(pattern, &arena, &ast, &parse_error);
  if (!iree_status_is_ok(status) && out_error) {
  ...
  if (iree_status_is_ok(status) &&
      (flags & IREE_TOKENIZER_UTIL_REGEX_COMPILE_FLAG_CASE_INSENSITIVE)) {

...
iree_status_t iree_hal_command_buffer_dispatch_validation(
  ...
  if (iree_hal_dispatch_uses_indirect_parameters(flags)) {
    ...
    if ((config.workgroup_count_ref.offset % sizeof(uint32_t)) != 0) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "workgroup count offset does not match the "
  ...
  if (iree_hal_dispatch_uses_indirect_arguments(flags)) {
    ...
    // If we find common traits of targets we want to verify here (alignment
    // being the most likely) we can try to check it here.
    if (IREE_UNLIKELY(bindings.count != 1)) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "a single binding must be specified when using indirect arguments");
    }
    const iree_hal_buffer_binding_requirements_t arguments_reqs = {
        .required_compatibility = IREE_HAL_BUFFER_COMPATIBILITY_QUEUE_DISPATCH,
        .usage = IREE_HAL_BUFFER_USAGE_DISPATCH_INDIRECT_PARAMETERS,
        .access = IREE_HAL_MEMORY_ACCESS_READ,
        .type = IREE_HAL_MEMORY_TYPE_DEVICE_VISIBLE,

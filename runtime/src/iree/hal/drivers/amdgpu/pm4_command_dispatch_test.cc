 ...
 protected:
  ...
  void SetUp() override {
    ...
    IREE_ASSERT_OK(LoadCtsExecutable(
        iree_hal_queue_family(test_device_.queue()),
  ...
  iree_status_t CreateIndirectCommandBuffer(
    ...
    IREE_RETURN_IF_ERROR(iree_hal_command_buffer_create(
        iree_hal_queue_family(test_device_.queue()), mode,
        IREE_HAL_COMMAND_CATEGORY_DISPATCH, binding_capacity,
        command_buffer.out()));
...
TEST_F(PM4CommandDispatchTest, RejectsImplicitBlockCountKernargs) {
  ...
  IREE_ASSERT_OK(LoadCtsExecutable(iree_hal_queue_family(test_device_.queue()),

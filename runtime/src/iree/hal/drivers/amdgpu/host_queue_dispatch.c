...
typedef struct iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t {
  // Common dispatch packet and kernarg plan.
  iree_hal_amdgpu_host_queue_dispatch_plan_t dispatch;
  // Number of extra kernarg blocks reserved for cooperative grid-sync state.
  uint32_t synchronization_block_count;
  ...
  uint64_t workitem_count;
  // True when this dispatch participates in a cooperative multi-grid launch.
  bool is_multi_grid;
} iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t;
...
iree_hal_amdgpu_host_queue_validate_cooperative_dispatch_flags(
    const iree_hal_amdgpu_host_queue_t* queue,
    iree_hal_dispatch_flags_t flags) {
  IREE_RETURN_IF_ERROR(
      iree_hal_amdgpu_host_queue_validate_dispatch_flags(flags));
  if (IREE_UNLIKELY(iree_hal_dispatch_uses_indirect_parameters(flags))) {
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED,
...
static iree_status_t
iree_hal_amdgpu_host_queue_prepare_cooperative_dispatch_plan(
    const iree_hal_amdgpu_host_queue_t* queue,
    iree_hal_executable_t* executable,
    iree_hal_executable_function_t export_ordinal,
    const iree_hal_dispatch_config_t config, iree_const_byte_span_t constants,
    ...
    const iree_hal_amdgpu_cooperative_grid_t* cooperative_grid,
    iree_hal_dispatch_flags_t flags,
    iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t* out_plan) {
  memset(out_plan, 0, sizeof(*out_plan));
  IREE_RETURN_IF_ERROR(
      iree_hal_amdgpu_host_queue_validate_cooperative_dispatch_flags(queue,
                                                                     flags));
  IREE_RETURN_IF_ERROR(
      iree_hal_amdgpu_host_queue_validate_cooperative_grid_config(
          cooperative_grid));
  ...
  out_plan->is_multi_grid = cooperative_grid->grid_count > 1;
  const bool has_implicit_args =
      out_plan->dispatch.custom_layout
          ? out_plan->dispatch.custom_layout->has_implicit_args
          : iree_any_bit_set(out_plan->dispatch.kernarg_layout->flags,
                             IREE_HAL_AMDGPU_KERNARG_LAYOUT_FLAG_IMPLICIT_ARGS);
  if (IREE_UNLIKELY(out_plan->is_multi_grid && !has_implicit_args)) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "cooperative multi-grid dispatch requires the executable hidden "
        "grid synchronization argument");
  }
  ...
  for (iree_host_size_t i = 0; i < 3; ++i) {
    ...
    if (IREE_UNLIKELY(
            out_plan->dispatch.kernel_args->workgroup_size[i] != 0 &&
            workitem_count >
                UINT64_MAX /
                    out_plan->dispatch.kernel_args->workgroup_size[i])) {
      return iree_make_status(
          IREE_STATUS_OUT_OF_RANGE,
          "cooperative grid workitem count exceeds uint64_t");
    }
    workitem_count *= out_plan->dispatch.kernel_args->workgroup_size[i];
  }
  ...
  out_plan->synchronization_block_count = has_implicit_args ? 1 : 0;
  out_plan->workgroup_count = (uint32_t)workgroup_count;
  out_plan->workitem_count = workitem_count * workgroup_count;
  if (IREE_UNLIKELY(out_plan->is_multi_grid &&
                    out_plan->workitem_count >
                        UINT64_MAX / cooperative_grid->grid_count)) {
    return iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "cooperative multi-grid workitem count exceeds uint64_t");
  }
  if (IREE_UNLIKELY(out_plan->synchronization_block_count != 0 &&
                    out_plan->dispatch.kernarg_block_count >=
                        queue->kernarg_ring.capacity)) {
    return iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "cooperative dispatch kernargs require %u blocks but the queue "
        "kernarg ring capacity is %u",
        out_plan->dispatch.kernarg_block_count + 1,
        queue->kernarg_ring.capacity);
  }
  if (out_plan->is_multi_grid &&
      !iree_any_bit_set(flags,
                        IREE_HAL_DISPATCH_FLAG_BORROW_RESOURCE_LIFETIMES)) {
    ++out_plan->dispatch.operation_resource_count;
  }
  return iree_ok_status();
}
...
iree_hal_amdgpu_host_queue_validate_dispatch_with_cooperative_grid(
    ...
    iree_host_size_t* out_operation_resource_count) {
  *out_operation_resource_count = 0;
  iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t plan;
  ...
  if (iree_status_is_ok(status) && plan.dispatch.uses_indirect_parameters) {
    status = iree_hal_amdgpu_host_queue_validate_dispatch_indirect_parameters(
        &config.workgroup_count_ref);
  }
  if (iree_status_is_ok(status)) {
    *out_operation_resource_count = plan.dispatch.operation_resource_count;
  }
  return status;
}
...
static iree_status_t iree_hal_amdgpu_host_queue_submit_dispatch_packets(
    ...
    iree_hal_executable_function_t export_ordinal,
    const iree_hal_dispatch_config_t config, iree_const_byte_span_t constants,
    const iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t*
        cooperative_plan,
    const iree_hal_amdgpu_cooperative_grid_t* cooperative_grid,
  ...
  const uint32_t cooperative_sync_block_count =
      cooperative_plan ? cooperative_plan->synchronization_block_count : 0;
  ...
  if (cooperative_sync_block_count != 0) {
    ...
    if (cooperative_plan->is_multi_grid) {
      ...
      sync_info->preceding_workitem_count =
          cooperative_plan->workitem_count * grid_ordinal;
      sync_info->total_workitem_count =
          cooperative_plan->workitem_count * grid_count;
    } else {
      sync_info->grid_count = 1;
      sync_info->total_workitem_count = cooperative_plan->workitem_count;
    }
    sync_info->workgroup_count = cooperative_plan->workgroup_count;
...
iree_hal_amdgpu_host_queue_submit_dispatch_internal(
    iree_hal_amdgpu_host_queue_t* queue,
    const iree_hal_amdgpu_wait_resolution_t* resolution,
    const iree_hal_semaphore_list_t signal_semaphore_list,
    iree_hal_executable_t* executable,
  ...
  *out_ready = false;
  ...
  iree_hal_amdgpu_host_queue_dispatch_plan_t dispatch_plan;
  iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t cooperative_plan;
  const iree_hal_amdgpu_host_queue_cooperative_dispatch_plan_t*
      cooperative_plan_ptr = NULL;
  if (cooperative_grid) {
    IREE_RETURN_IF_ERROR(
        iree_hal_amdgpu_host_queue_prepare_cooperative_dispatch_plan(
            queue, executable, export_ordinal, config, constants, bindings,
            cooperative_grid, flags, &cooperative_plan));
    dispatch_plan = cooperative_plan.dispatch;
    cooperative_plan_ptr = &cooperative_plan;
  } else {
    IREE_RETURN_IF_ERROR(iree_hal_amdgpu_host_queue_prepare_dispatch_plan(
        queue, executable, export_ordinal, config, constants, bindings, flags,
        &dispatch_plan));
  ...
  if (iree_status_is_ok(status) && dispatch_plan.uses_indirect_parameters) {
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_amdgpu_host_queue_submit_dispatch_packets(
        queue, resolution, signal_semaphore_list, &dispatch_plan, executable,
        export_ordinal, config, constants, cooperative_plan_ptr,
        cooperative_grid, binding_ptrs, workgroup_count_ptr,
        cooperative_sync_ptr, operation_resources, uses_custom_direct_arguments,
...
iree_status_t iree_hal_amdgpu_host_queue_submit_dispatch(
    ...
    iree_hal_amdgpu_host_queue_submission_flags_t submission_flags,
    bool* out_ready) {
  return iree_hal_amdgpu_host_queue_submit_dispatch_internal(
      queue, resolution, signal_semaphore_list, executable, export_ordinal,
      config, constants, bindings, /*cooperative_grid=*/NULL, flags,
      submission_flags, out_ready);
}

iree_status_t iree_hal_amdgpu_host_queue_submit_dispatch_cooperative(
    iree_hal_amdgpu_host_queue_t* queue,
    const iree_hal_amdgpu_wait_resolution_t* resolution,
    const iree_hal_semaphore_list_t signal_semaphore_list,
    iree_hal_executable_t* executable,
  ...
  return iree_hal_amdgpu_host_queue_submit_dispatch_internal(
      queue, resolution, signal_semaphore_list, executable, export_ordinal,
      config, constants, bindings, cooperative_grid, flags, submission_flags,
      out_ready);
}

...
typedef struct iree_async_proactor_io_uring_t {
  ...
  // TID of the poll thread during active CQE processing, 0 otherwise.
  //
  // Set at the start of the CQE processing phase (after wait returns) and
  // cleared after the drain loop completes. Used by submit()'s Phase 4 to
  // decide how to flush:
  //
  ...
  //                              will flush pending SQEs during its drain).
  //
  //   poll_tid == 0           → poll thread idle: wake it to flush.
  //
  // The CQE processing loop uses a CQ tail snapshot to prevent processing
  // CQEs generated by Phase 4 flushes in the same iteration (which would
  // create infinite re-submission loops for operations that complete inline).

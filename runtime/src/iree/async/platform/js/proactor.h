...
// Registers an externally completed operation and returns the JS completion
// token used to complete it through the proactor ring.
//
// This is for browser/Node APIs that are initiated outside the generic async
// operation submit path but still need completion dispatch through
// iree_async_proactor_poll(). The operation's resources are retained until JS
// reports completion for |out_token|.

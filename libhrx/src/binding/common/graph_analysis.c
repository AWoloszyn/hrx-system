...
// Returns true if the node can execute inside a command buffer. Cooperative
// kernels require queue selection at submission time and therefore remain
// explicit queue dispatch operations in executable graphs.
...
static iree_uint32x2_t iree_hal_streaming_graph_partition_with_streams(
  ...
  for (uint32_t i = 0; i < node_count;) {
    bool is_recordable = iree_hal_streaming_graph_node_can_record(&nodes[i]);
    if (!is_recordable) {

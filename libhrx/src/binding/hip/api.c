...
static hipError_t iree_hip_current_mem_pool(
  ...
  if (!context || !context->device_entry) {
    return hipErrorInvalidDevice;
  }
  hrx_mem_pool_t pool =
...
static hipError_t iree_hip_context_total_memory_from_spec(
  ...
  if (!context || !out_known || !out_total) {
    return hipErrorInvalidDevice;
  }
  *out_known = false;
  *out_total = 0;
...
static bool iree_hip_managed_memory_page_range(
  ...
  *out_page_count = 0;
  ...
  if (buffer_ref->offset > IREE_DEVICE_SIZE_MAX - count) {
    return false;
  }
...
static int32_t iree_hip_managed_uniform_location(const int32_t* locations,
  ...
  for (iree_host_size_t i = 1; i < page_count; ++i) {
    if (locations[first_page + i] != location) {
...
static hipError_t iree_hip_array_desc_to_format(
  ...
  if (out_num_channels) {
    *out_num_channels = channel_count;
  }
  if (out_element_size) {
...
static hipError_t iree_hip_array_format_to_desc(hipArray_Format format,
  ...
  if (out_element_size) {
    *out_element_size = ((size_t)bits * num_channels) / 8;
  }
  return hipSuccess;
}
...
static hipError_t iree_hip_array_byte_range_to_elements_for_array(
    ...
    size_t* out_element_offset, size_t* out_element_width) {
  if (out_element_offset) {
    *out_element_offset = 0;
  }
  if (out_element_width) {
    *out_element_width = 0;
  }
  if (!array_info) {
    return hipErrorInvalidValue;
  ...
  if (out_element_width) {
    *out_element_width = element_width;
  }
  return hipSuccess;
}
...
static hipError_t iree_hip_array_legacy_row_range(
  ...
  if (out_array) {
    *out_array = array_info;
  }
  if (out_device_ptr) {
    *out_device_ptr = array_info->device_ptr + byte_offset;
  }
  if (!out_array) {
    iree_hip_array_release(array_info);
  }
...
static hipError_t iree_hip_array_validate_extent_limits(hipExtent extent) {
  ...
  if (extent.height == 0 && extent.depth != 0) {
    return hipErrorInvalidValue;
  }
  if (extent.height == 0) {
...
static hipError_t iree_hip_memcpy_peer_staged(
  ...
  if (!dst_context || !src_context || !dst || !src) {
    return hipErrorInvalidValue;
  }
...
HIPAPI hipError_t hipArrayGetInfo(hipChannelFormatDesc* desc, hipExtent* extent,
  ...
  if (extent) {
    *extent = array_info->public_extent;
  }
  if (flags) {
...
// Resolves a registered variable to its device payload while retaining the
// module that owns the allocation. Callers must keep |out_module| retained
// until they have finished consuming the returned address.
static hipError_t iree_hip_variable_symbol_lookup(
    const void* symbol, iree_hal_streaming_deviceptr_t* out_device_address,
    iree_device_size_t* out_size_bytes,
    iree_hal_streaming_module_t** out_module) {
  if (out_device_address) *out_device_address = 0;
  ...
  *out_module = NULL;
  ...
  iree_hal_streaming_symbol_t* resolved_symbol = NULL;
  iree_hal_streaming_module_t* module = NULL;
  iree_status_t status = iree_hal_streaming_context_symbol_map_lookup(
      &context->symbol_map, (void*)symbol, &resolved_symbol, &module);
  if (!iree_status_is_ok(status) || !resolved_symbol) {
    iree_status_ignore(status);
    return hipErrorInvalidSymbol;
  }
  if (resolved_symbol->type != IREE_HAL_STREAMING_SYMBOL_TYPE_GLOBAL &&
      resolved_symbol->type != IREE_HAL_STREAMING_SYMBOL_TYPE_DATA) {
    iree_hal_streaming_module_release(module);
    return hipErrorInvalidSymbol;
  }

  if (out_device_address) {
    *out_device_address = resolved_symbol->managed_buffer
                              ? resolved_symbol->managed_buffer->device_ptr
                              : resolved_symbol->device_address;
  }
  if (out_size_bytes) {
    *out_size_bytes = resolved_symbol->managed_buffer
                          ? resolved_symbol->managed_buffer->logical_size
                          : resolved_symbol->size_bytes;
  }
  *out_module = module;
  return hipSuccess;
}
...
HIPAPI hipError_t hipGetSymbolAddress(void** devPtr, const void* symbol) {
  ...
  iree_hal_streaming_deviceptr_t device_address = 0;
  iree_hal_streaming_module_t* module = NULL;
  hipError_t result = iree_hip_variable_symbol_lookup(
      symbol, &device_address, /*out_size_bytes=*/NULL, &module);
  if (result != hipSuccess) {
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(result);
  }

  if (IREE_UNLIKELY(device_address > UINTPTR_MAX)) {
    iree_hal_streaming_module_release(module);
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
...
HIPAPI hipError_t hipGetSymbolSize(size_t* size, const void* symbol) {
  ...
  iree_device_size_t size_bytes = 0;
  iree_hal_streaming_module_t* module = NULL;
  hipError_t result = iree_hip_variable_symbol_lookup(
      symbol, /*out_device_address=*/NULL, &size_bytes, &module);
  if (result != hipSuccess) {
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(result);
  }

  if (IREE_UNLIKELY(size_bytes > SIZE_MAX)) {
    iree_hal_streaming_module_release(module);
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
...
HIPAPI hipError_t hipMemcpyToSymbolAsync(const void* symbol, const void* src,
  ...
  iree_hal_streaming_deviceptr_t device_address = 0;
  iree_device_size_t symbol_size = 0;
  iree_hal_streaming_module_t* module = NULL;
  hipError_t result = iree_hip_variable_symbol_lookup(symbol, &device_address,
                                                      &symbol_size, &module);
  if (result == hipSuccess &&
      (device_address > UINTPTR_MAX || symbol_size > SIZE_MAX ||
       offset > (size_t)symbol_size ||
       sizeBytes > (size_t)symbol_size - offset)) {
    result = hipErrorInvalidValue;
  }
  if (result == hipSuccess) {
    result = hipMemcpyAsync((uint8_t*)(uintptr_t)device_address + offset, src,
                            sizeBytes, hipMemcpyHostToDevice, stream);
  }
  iree_hal_streaming_module_release(module);
...
HIPAPI hipError_t hipMemcpyFromSymbolAsync(void* dst, const void* symbol,
  ...
  if (result == hipSuccess) {
    result =
        hipMemcpyAsync(dst, (const uint8_t*)(uintptr_t)device_address + offset,
                       sizeBytes, hipMemcpyDeviceToHost, stream);
  }
  iree_hal_streaming_module_release(module);
...
static hipError_t iree_hip_enqueue_stream_value_write(
  ...
  if ((uintptr_t)ptr % byte_length != 0) {
    return hipErrorInvalidValue;
  }
...
// Creates a function handle only while |module_handle| is still registered.
// This lock also orders handle creation before or after module retirement.
static hipError_t iree_hip_live_module_create_function_handle(
    hipModule_t module_handle, iree_hal_streaming_symbol_t* symbol,
    hipFunction_t* out_function) {
  *out_function = NULL;
  iree_hip_live_module_ensure_initialized();
  iree_slim_mutex_lock(&iree_hip_live_module_mutex);
  iree_hal_streaming_module_t* module = NULL;
  for (iree_hip_live_module_entry_t* entry = iree_hip_live_module_head; entry;
       entry = entry->next) {
    if ((hipModule_t)entry->module == module_handle) {
      module = entry->module;
      break;
    }
  }
...
//
// Supported formats:
// - AMDGPU ELF code objects.
// - Clang offload bundles, compressed code-object bundles, and HIP wrappers.
//
...
hipError_t iree_hip_module_load_data_span(iree_const_byte_span_t image,
  ...
  *out_module = NULL;
  ...
  iree_status_t status = iree_hal_streaming_module_create_from_memory(
      context, IREE_HAL_EXECUTABLE_LOAD_FLAG_ALLOW_OPTIMIZATION, image,
      context->host_allocator, &stream_module);
  if (iree_status_is_ok(status)) {
    status = iree_hip_live_module_insert(stream_module);
  }
  if (iree_status_is_ok(status)) {
    *out_module = (hipModule_t)stream_module;
  } else {
    iree_hal_streaming_module_release(stream_module);
  }
  return iree_module_status_to_hip_result(status);
...
HIPAPI hipError_t hipModuleGetFunction(hipFunction_t* function,
  ...
  *function = NULL;
  ...
  if (iree_status_is_ok(status)) {
    hipError_t handle_result = iree_hip_live_module_create_function_handle(
        module, stream_symbol, function);
    if (handle_result != hipSuccess) {
      iree_hal_streaming_module_release(stream_module);
      HIP_RETURN_ERROR(handle_result);
    }
    HIP_DEBUG_LOG(
        "[HIP_API] hipModuleGetFunction: found symbol %p -> handle %p\n",
...
HIPAPI hipError_t hipModuleGetGlobal(hipDeviceptr_t* dptr, size_t* bytes,
  ...
  if (iree_status_is_ok(status) &&
      (device_ptr > UINTPTR_MAX || size > SIZE_MAX)) {
    status = iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "module global address or size exceeds the host ABI width");
  }
  if (iree_status_is_ok(status)) {
    if (dptr) {
...
HIPAPI hipError_t hipGetFuncBySymbol(hipFunction_t* functionPtr,
  ...
  *functionPtr = NULL;
  ...
  iree_hal_streaming_module_t* module = NULL;
  iree_status_t status = iree_hal_streaming_context_symbol_map_lookup(
      &context->symbol_map, (void*)symbolPtr, &symbol, &module);
  if (!iree_status_is_ok(status)) {
    iree_status_ignore(status);
  ...
  // Compiler-registered host function addresses already resolve through the
  // context symbol map, which retains their registration-owned module under
  // the map lock. Returning that address avoids creating a second lifetime
  // domain for the same compiler registration.
  *functionPtr = (hipFunction_t)symbolPtr;
  iree_hal_streaming_module_release(module);
  return hipSuccess;
}
...
static hipError_t iree_hip_resolve_function_for_metadata(
  ...
  *out_module = NULL;
  ...
  hipError_t result = iree_hip_ensure_context(&context);
  if (result != hipSuccess) return result;
  result = iree_hip_resolve_function_symbol(context, function_address,
                                            out_symbol, out_module);
...
HIPAPI hipError_t hipFuncGetAttribute(int* pi, hipFuncAttribute_t attrib,
  ...
  result = iree_hip_function_attribute(symbol, attrib, pi);
  iree_hal_streaming_module_release(module);
  HIP_RETURN_ERROR(result);
}
...
HIPAPI hipError_t hipFuncGetAttributes(hipFuncAttributes* attr,
  ...
  hipError_t result = iree_hip_resolve_function_for_metadata(
      ...
      hipErrorInvalidDeviceFunction, &symbol, &module);
  if (result != hipSuccess) HIP_RETURN_ERROR(result);
  // The aggregate API cannot represent an unknown field separately. Leave the
  // caller's structure untouched rather than returning a plausible but false
...
HIPAPI hipError_t hipFuncSetCacheConfig(hipFunction_t hfunc,
  ...
  switch (config) {
    ...
    case hipFuncCachePreferL1:
      // The smallest nonzero carveout requests the largest available cache.
...
// module function handle to its binding-owned symbol. On success,
// |out_module| retains the symbol and must be released by the caller. Both
// outputs are set to NULL on failure.
static hipError_t iree_hip_resolve_function_symbol(
    iree_hal_streaming_context_t* context, const void* function_address,
    iree_hal_streaming_symbol_t** out_symbol,
    iree_hal_streaming_module_t** out_module) {
  *out_symbol = NULL;
  *out_module = NULL;
  if (!context || !function_address) {
    return hipErrorInvalidValue;
  }

  iree_hal_streaming_symbol_t* symbol = NULL;
  iree_hal_streaming_module_t* module = NULL;
  if (!iree_hip_function_handle_lookup(function_address, &symbol, &module)) {
    iree_status_t lookup_status = iree_hal_streaming_context_symbol_map_lookup(
        &context->symbol_map, (void*)function_address, &symbol, &module);
  ...
  if (!module->context || module->context != context) {
    iree_hal_streaming_module_release(module);
    return hipErrorInvalidDevice;
  }
  *out_symbol = symbol;
...
typedef struct iree_hip_launch_events_t {
  // Optional start event retained through launch recording.
  iree_hal_streaming_event_t* start;
  // Optional stop event retained through launch recording.
  iree_hal_streaming_event_t* stop;
} iree_hip_launch_events_t;

static void iree_hip_launch_events_release(iree_hip_launch_events_t* events) {
  iree_hal_streaming_event_release(events->stop);
  iree_hal_streaming_event_release(events->start);
  memset(events, 0, sizeof(*events));
}

static hipError_t iree_hip_launch_events_acquire(
    hipEvent_t start_event, hipEvent_t stop_event,
    iree_hal_streaming_context_t* context,
    iree_hip_launch_events_t* out_events) {
  memset(out_events, 0, sizeof(*out_events));
  hipError_t result = hipSuccess;
  if (start_event) {
    result = iree_hip_event_lookup_retain(start_event, &out_events->start);
  }
  if (result == hipSuccess && stop_event) {
    result = iree_hip_event_lookup_retain(stop_event, &out_events->stop);
  }
  if (result == hipSuccess &&
      ((out_events->start && out_events->start->context != context) ||
       (out_events->stop && out_events->stop->context != context))) {
    result = hipErrorInvalidHandle;
  }
  if (result != hipSuccess) iree_hip_launch_events_release(out_events);
  return result;
}

static hipError_t iree_hip_launch_event_record(
    iree_hal_streaming_event_t* event, iree_hal_streaming_stream_t* stream) {
  if (!event) return hipSuccess;
...
static hipError_t iree_hip_validate_launch_arguments_before_events(
  ...
  if (!events->start && !events->stop) return hipSuccess;
  iree_status_t status = iree_ok_status();
  if (iree_any_bit_set(params->flags,
                       IREE_HAL_STREAMING_DISPATCH_FLAG_PRE_PACKED)) {
    status =
        iree_hal_streaming_validate_prepacked_kernel_arguments(symbol, params);
  } else if (iree_any_bit_set(params->flags,
                              IREE_HAL_STREAMING_DISPATCH_FLAG_ARGS_ARRAY)) {
    status = iree_hal_streaming_validate_raw_argument_list(
...
static hipError_t iree_hip_launch_kernel_on_stream(
  ...
  if (result == hipSuccess) {
    ...
    result = iree_hip_validate_launch_arguments_before_events(symbol, &params,
                                                              events);
    if (result == hipSuccess) {
...
HIPAPI hipError_t hipExtLaunchMultiKernelMultiDevice(
  ...
  iree_host_size_t modules_offset = 0;
  ...
  if (!iree_host_size_checked_mul(launch_count,
      ...
      !iree_host_size_checked_add(launches_offset, launches_size,
                                  &modules_offset) ||
      !iree_host_size_checked_align(modules_offset,
                                    iree_alignof(iree_hal_streaming_module_t*),
                                    &modules_offset) ||
      !iree_host_size_checked_mul(launch_count, sizeof(void*), &modules_size) ||
      !iree_host_size_checked_add(modules_offset, modules_size,
                                  &allocation_size)) {
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(hipErrorOutOfMemory);
  }
  ...
  iree_hal_streaming_module_t** modules =
      (iree_hal_streaming_module_t**)(allocation + modules_offset);
  ...
  for (iree_host_size_t i = 0; i < launch_count; ++i) {
    iree_hal_streaming_module_release(modules[i]);
  }
...
//  - kernelParams: [IN] One pointer to each kernel argument value.
//  - extra: [IN] Packed launch-buffer options, or NULL.
//
// Returns:
//  - hipSuccess: Kernel launched successfully.
...
static hipError_t iree_hip_module_launch_kernel(
    ...
    hipEvent_t stop_event, const uint32_t exact_workitem_count[3],
    const uint32_t validation_block_dim[3]) {
  IREE_TRACE_ZONE_BEGIN(z0);
  HIP_DEBUG_LOG(
...
#if IREE_HIP_VERBOSE_DEBUG
  // Dump packed launch-buffer contents when verbose diagnostics are enabled.
  void* debug_buf = NULL;
  size_t debug_sz = 0;
  if (extra) {
    ...
    // Interpret the first 64 bytes as pointer-sized words.
  ...
  if (result == hipSuccess) {
    ...
    iree_hal_streaming_context_t* current_context =
        iree_hal_streaming_context_current();
    if (!current_context ||
        current_context->device_ordinal != context->device_ordinal) {
      result = hipErrorInvalidDevice;
    }
  }
  if (result == hipSuccess) {
...
HIPAPI hipError_t hipModuleLaunchKernel(
  ...
  return iree_hip_module_launch_kernel(
      ...
      /*exact_workitem_count=*/NULL, /*validation_block_dim=*/NULL);
}
...
//   gridDim = ceil(globalWorkSize / localWorkSize)
//
// Parameters:
...
//
// Non-uniform kernels preserve the exact global extent in the dispatch packet
// so the final workgroup may contain fewer active workitems.
HIPAPI hipError_t hipExtModuleLaunchKernel(
  ...
  const uint32_t effective_block_dim[3] = {
      iree_min(globalWorkSizeX, localWorkSizeX),
      iree_min(globalWorkSizeY, localWorkSizeY),
      iree_min(globalWorkSizeZ, localWorkSizeZ),
  };
  const bool has_partial_workgroup =
      (globalWorkSizeX % effective_block_dim[0]) ||
      (globalWorkSizeY % effective_block_dim[1]) ||
      (globalWorkSizeZ % effective_block_dim[2]);

  const unsigned int gridDimX = globalWorkSizeX / effective_block_dim[0] +
                                (globalWorkSizeX % effective_block_dim[0] != 0);
  const unsigned int gridDimY = globalWorkSizeY / effective_block_dim[1] +
                                (globalWorkSizeY % effective_block_dim[1] != 0);
  const unsigned int gridDimZ = globalWorkSizeZ / effective_block_dim[2] +
                                (globalWorkSizeZ % effective_block_dim[2] != 0);
  const uint32_t exact_workitem_count[3] = {
      globalWorkSizeX,
      globalWorkSizeY,
      globalWorkSizeZ,
  };
...
HIPAPI hipError_t hipModuleLaunchCooperativeKernel(
  ...
  iree_hal_streaming_module_t* module = NULL;
  hipError_t result =
      iree_hip_resolve_function_symbol(context, f, &symbol, &module);
  if (result != hipSuccess || symbol->module->context != context) {
    iree_hal_streaming_module_release(module);
    iree_hip_resolved_stream_release(&resolved_stream);
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(result == hipSuccess ? hipErrorInvalidDevice
...
static hipError_t iree_hip_prefetch_batch_location_for_operation(
  ...
  for (size_t i = 1; i < num_prefetch_locs; ++i) {
    if (prefetch_loc_idxs[i] > operation) {
      break;
    }
    location_index = i;
  }
...
static bool iree_hip_pointer_is_registered_managed_variable(const void* ptr,
  ...
  if (symbol_type != IREE_HAL_STREAMING_SYMBOL_TYPE_DATA) {
    return false;
  }
  if (out_size) {
    *out_size = symbol_size;
  }
  return true;
}
...
static hipError_t iree_hip_resolve_graph_exec(
  ...
  for (iree_hip_live_graph_exec_entry_t* current =
           iree_hip_live_graph_exec_head;
       current; current = current->next) {
    if (current->graph_exec != graphExec) {
      continue;
    }
    iree_hal_streaming_graph_exec_t* exec =
...
HIPAPI hipError_t hipGraphAddKernelNode(hipGraphNode_t* pGraphNode,
  ...
  hipError_t symbol_result = iree_hip_resolve_function_symbol(
      stream_graph->context, params->func, &symbol, &module);
  ...
  iree_status_t add_status = iree_hal_streaming_graph_add_kernel_node(
...
HIPAPI hipError_t hipGraphGetNodes(hipGraph_t graph, hipGraphNode_t* pNodes,
  ...
  for (iree_hal_streaming_node_block_t* block = stream_graph->node_blocks;
    ...
    for (size_t i = 0; i < block->count; ++i) {
      if (!iree_hip_graph_node_is_hidden(block->nodes[i])) {
        ++total_count;
      }
    }
  }
...
HIPAPI hipError_t hipGraphGetEdges(hipGraph_t graph, hipGraphNode_t* from,
  ...
  for (iree_hal_streaming_graph_edge_t* edge = stream_graph->additional_edges;
       edge; edge = edge->next) {
    if (iree_hip_graph_edge_is_hidden(edge->from, edge->to)) {
      continue;
    }
    if (copied_count < capacity) {
...
HIPAPI hipError_t hipGraphNodeGetDependentNodes(hipGraphNode_t node,
  ...
  for (iree_hal_streaming_node_block_t* block = graph->node_blocks; block;
    ...
    for (size_t i = 0; i < block->count; ++i) {
      ...
      if (iree_hip_graph_node_is_hidden(candidate)) {
        continue;
      }
      for (uint32_t j = 0; j < candidate->dependency_count; ++j) {
        if (candidate->dependencies[j] != stream_node) {
          continue;
        }
        if (iree_hip_graph_edge_is_hidden(stream_node, candidate)) {
          continue;
        }
        if (copied_count < capacity) {
    ...
    for (iree_host_size_t i = 0; i < block->count; ++i) {
      ...
      for (uint32_t j = 0; j < node->dependency_count; ++j) {
        const iree_hal_streaming_graph_node_t* dependency =
            node->dependencies[j];
        if (iree_hip_graph_edge_is_hidden(dependency, node)) {
          continue;
        }
        if (fprintf(file,
...
static hipError_t iree_hip_resolve_symbol_copy_range(const void* symbol,
  ...
  *out_symbol_ptr = NULL;
  ...
  if (count == 0 || offset > SIZE_MAX - count) {
    return hipErrorInvalidValue;
  }
...
static hipError_t iree_hip_graph_validate_batch_mem_op_node_params(
  ...
  if (out_param_array_size) {
    *out_param_array_size = 0;
  }
  ...
  if (out_param_array_size) {
    *out_param_array_size = param_array_size;
  }
  return hipSuccess;
}
...
static hipError_t iree_hip_graph_validate_memset_params(
  ...
  if (out_dst_ref) {
    memset(out_dst_ref, 0, sizeof(*out_dst_ref));
  }
...
static hipError_t iree_hip_graph_validate_memcpy3d_params(
  ...
  if (out_dst_span) {
    *out_dst_span = dst_span;
  }
  if (out_src_span) {
    *out_src_span = src_span;
  }
  return hipSuccess;
}
...
iree_hip_graph_find_post_memcpy_callback(
  ...
  for (iree_hal_streaming_node_block_t* block = graph->node_blocks; block;
    ...
    for (iree_host_size_t i = 0; i < block->count; ++i) {
      iree_hal_streaming_graph_node_t* candidate = block->nodes[i];
      if (!iree_hip_graph_node_is_hidden_memcpy_callback(candidate)) {
        continue;
      }
      for (uint32_t j = 0; j < candidate->dependency_count; ++j) {
        if (candidate->dependencies[j] == node) {
          return candidate;
        }
      }
    }
...
hipGraphExecKernelNodeSetParams(hipGraphExec_t graphExec, hipGraphNode_t node,
  ...
  // Hold the old module independently while the common setter transfers the
  // node's ownership to the candidate module. On restore this retained
  // reference becomes the node's ownership again.
  iree_hal_streaming_module_retain(old_attrs.module);
  ...
  if (old_attrs.constants_capacity > 0 && old_attrs.constants.data) {
    ...
    if (!iree_status_is_ok(status)) {
      iree_status_ignore(status);
      iree_hal_streaming_module_release(old_attrs.module);
      iree_hal_streaming_graph_exec_release(exec);
      HIP_RETURN_ERROR(hipErrorOutOfMemory);
    }
  ...
  result = iree_hip_graph_exec_rebuild(exec);
  iree_hal_streaming_module_t* replacement_module =
      stream_node->attrs.kernel.module;
...
HIPAPI hipError_t hipGraphKernelNodeGetParams(hipGraphNode_t node,
  ...
  iree_hal_streaming_graph_kernel_node_attrs_t* attrs =
      &stream_node->attrs.kernel;
  if (attrs->constants.data_length > 0) {
    attrs->hip_argument_size = attrs->constants.data_length;
    attrs->hip_extra_storage[0] = HIP_LAUNCH_PARAM_BUFFER_POINTER;
    attrs->hip_extra_storage[1] = (void*)attrs->constants.data;
    attrs->hip_extra_storage[2] = HIP_LAUNCH_PARAM_BUFFER_SIZE;
    attrs->hip_extra_storage[3] = &attrs->hip_argument_size;
    attrs->hip_extra_storage[4] = HIP_LAUNCH_PARAM_END;
    params->extra = attrs->hip_extra_storage;
  } else {
    params->extra = NULL;
  }
  return hipSuccess;
...
HIPAPI hipError_t hipGraphKernelNodeSetParams(hipGraphNode_t node,
  ...
  const bool preserves_exact_extent =
      stream_node->attrs.kernel.workitem_count[0] != 0 ||
      stream_node->attrs.kernel.workitem_count[1] != 0 ||
      stream_node->attrs.kernel.workitem_count[2] != 0;
  if (preserves_exact_extent &&
      (params->gridDim.x != stream_node->attrs.kernel.grid_dim[0] ||
       params->gridDim.y != stream_node->attrs.kernel.grid_dim[1] ||
       params->gridDim.z != stream_node->attrs.kernel.grid_dim[2] ||
       params->blockDim.x != stream_node->attrs.kernel.block_dim[0] ||
       params->blockDim.y != stream_node->attrs.kernel.block_dim[1] ||
       params->blockDim.z != stream_node->attrs.kernel.block_dim[2])) {
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  ...
  iree_hal_streaming_module_t* module = NULL;
  hipError_t result = iree_hip_resolve_function_symbol(
      stream_node->graph->context, params->func, &symbol, &module);
  if (result != hipSuccess) {
    HIP_RETURN_ERROR(result);
  }
  if (!params->kernelParams && !params->extra &&
  ...
  iree_hal_streaming_dispatch_params_t dispatch_params = {
      ...
      .workitem_count =
          {
              stream_node->attrs.kernel.workitem_count[0],
              stream_node->attrs.kernel.workitem_count[1],
              stream_node->attrs.kernel.workitem_count[2],
          },
      .shared_memory_bytes = params->sharedMemBytes,
      .buffer = params_ptr,
      .buffer_size = params_size,
...
static hipError_t iree_hip_graph_set_host_memcpy_callback_params(
  ...
  if (out_old_callback_data) {
    *out_old_callback_data = old_callback_data;
  }
  result = iree_hip_graph_configure_memcpy_callback_data(
...
HIPAPI hipError_t hipGetDriverEntryPoint(const char* symbol, void** function,
  ...
  HIP_RETURN_ERROR(iree_hip_lookup_runtime_symbol(
...
HIPAPI hipError_t hipGetDriverEntryPoint_spt(const char* symbol,
  ...
  HIP_RETURN_ERROR(
      iree_hip_lookup_runtime_symbol(symbol, prefer_spt, function, status));
}
...
HIPAPI void __hipRegisterManagedVar(void* hipModule, void** pointer,
  ...
  void* initial_pointer = init_value ? init_value : *pointer;
  if (!initial_pointer) return;
  ...
  iree_status_t status =
      iree_hal_streaming_global_symbol_registry_insert_managed_variable(
          ...
          initial_pointer, pointer, name, size, align);
  iree_status_ignore(status);
}

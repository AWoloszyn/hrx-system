...
enum {
  HRX_HIP_STREAM_WAIT_VALUE_GTE = 0,
  HRX_HIP_STREAM_WAIT_VALUE_EQ = 1,
  HRX_HIP_STREAM_WAIT_VALUE_AND = 2,
  HRX_HIP_STREAM_WAIT_VALUE_NOR = 3,
  HRX_HIP_STREAM_WRITE_VALUE_DEFAULT = 0,
  HRX_HIP_STREAM_WRITE_VALUE_INCREMENT = 0x1000,
  HRX_HIP_STREAM_WRITE_VALUE_DECREMENT = 0x1001,
};
...
static hipError_t iree_module_file_status_to_hip_result(iree_status_t status) {
  if (iree_status_is_ok(status)) {
    return hipSuccess;
  }
  ...
  switch (code) {
    ...
    case IREE_STATUS_INVALID_ARGUMENT:
      ...
      return hipErrorInvalidImage;
    default:
      return iree_module_status_to_hip_result(status);
  }
}
...
static bool iree_hip_graph_node_is_active(
    const iree_hal_streaming_graph_node_t* node);
static hipError_t iree_hip_graph_set_kernel_node_params(
    iree_hal_streaming_graph_node_t* node, const hipKernelNodeParams* params);
...
HIPAPI hipError_t hipDeviceGraphMemTrim(int device) {
  ...
  iree_status_t status = iree_hal_streaming_graph_memory_trim(device_entry);
  return iree_status_to_fixed_hip_result(status, hipErrorUnknown);
}
  ...
  *out_params = *params;
  ...
  if (result == hipSuccess && src_array && dst_array) {
    if (src_array->element_size != dst_array->element_size ||
        src_array->extent.width != dst_array->extent.width ||
        src_array->extent.height != dst_array->extent.height ||
        src_array->extent.depth != dst_array->extent.depth) {
      result = hipErrorInvalidValue;
    }
  }
...
static hipError_t iree_hip_memset_2d(void* dst, size_t pitch, int value,
  ...
  iree_host_size_t slice_pitch = 0;
  if (IREE_UNLIKELY(!iree_host_size_checked_mul(pitch, height, &slice_pitch))) {
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  ...
  iree_status_t status = iree_hal_streaming_memory_memset_3d(
      context, (iree_hal_streaming_deviceptr_t)dst, pitch, slice_pitch, width,
      height, /*depth=*/1, &value, 1, stream_obj);
  if (!iree_status_is_ok(status)) {
  ...
  if (!is_async) {
    status = iree_hal_streaming_memory_complete_synchronous_memset(
        context, (iree_hal_streaming_deviceptr_t)dst, byte_span, stream_obj);
    if (!iree_status_is_ok(status)) {
      hipError_t result = iree_memset_status_to_hip_result(status);
      iree_hip_resolved_stream_release(&resolved_stream);
      IREE_TRACE_ZONE_END(z0);
...
typedef struct iree_hip_live_module_entry_t {
  // Loaded module represented by the public handle.
  iree_hal_streaming_module_t* module;
  // Allocator used for this registry entry.
  iree_allocator_t host_allocator;
  // Next live public module handle.
  struct iree_hip_live_module_entry_t* next;
} iree_hip_live_module_entry_t;

// Module handles expose the streaming module address, so callers must validate
// the address against this registry before dereferencing it. Query operations
// retain the module while holding the registry lock. Unload removes the handle
// under the same lock before dropping the owning reference, ensuring that a
// concurrent query either acquires a live module or rejects the stale handle.
static iree_once_flag iree_hip_live_module_mutex_once = IREE_ONCE_FLAG_INIT;
static iree_slim_mutex_t iree_hip_live_module_mutex;
static iree_hip_live_module_entry_t* iree_hip_live_module_head;

static void iree_hip_live_module_mutex_initialize(void) {
  iree_slim_mutex_initialize(&iree_hip_live_module_mutex);
}

static void iree_hip_live_module_ensure_initialized(void) {
  iree_call_once(&iree_hip_live_module_mutex_once,
                 iree_hip_live_module_mutex_initialize);
}

static iree_status_t iree_hip_live_module_insert(
    iree_hal_streaming_module_t* module) {
  iree_hip_live_module_entry_t* entry = NULL;
  IREE_RETURN_IF_ERROR(iree_allocator_malloc(module->host_allocator,
                                             sizeof(*entry), (void**)&entry));
  entry->module = module;
  entry->host_allocator = module->host_allocator;

  iree_hip_live_module_ensure_initialized();
  iree_slim_mutex_lock(&iree_hip_live_module_mutex);
  entry->next = iree_hip_live_module_head;
  iree_hip_live_module_head = entry;
  iree_slim_mutex_unlock(&iree_hip_live_module_mutex);
  return iree_ok_status();
}

static hipError_t iree_hip_live_module_acquire(
  ...
  *out_module = NULL;
  ...
  iree_hip_live_module_ensure_initialized();
  iree_slim_mutex_lock(&iree_hip_live_module_mutex);
  for (iree_hip_live_module_entry_t* entry = iree_hip_live_module_head; entry;
       entry = entry->next) {
    if ((hipModule_t)entry->module != handle) continue;
    iree_hal_streaming_module_retain(entry->module);
    *out_module = entry->module;
    break;
  }
  iree_slim_mutex_unlock(&iree_hip_live_module_mutex);
  return *out_module ? hipSuccess : hipErrorInvalidResourceHandle;
}

static hipError_t iree_hip_live_module_remove(
    hipModule_t handle, iree_hal_streaming_module_t** out_module) {
  *out_module = NULL;
  if (!handle) return hipErrorInvalidResourceHandle;
  ...
  iree_hip_live_module_ensure_initialized();
  iree_slim_mutex_lock(&iree_hip_live_module_mutex);
  iree_hip_live_module_entry_t** link = &iree_hip_live_module_head;
  while (*link && (hipModule_t)(*link)->module != handle) {
    link = &(*link)->next;
  }
  iree_hip_live_module_entry_t* entry = *link;
  if (entry) {
    *link = entry->next;
    *out_module = entry->module;
  }
  ...
  if (!entry) return hipErrorNotFound;
  iree_allocator_free(entry->host_allocator, entry);
  return hipSuccess;
}
...
HIPAPI hipError_t hipModuleLoad(hipModule_t* module, const char* fname) {
  ...
  *module = NULL;
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hip_live_module_insert(stream_module);
  }
  if (iree_status_is_ok(status)) {
    *module = (hipModule_t)stream_module;
  } else {
    iree_hal_streaming_module_release(stream_module);
  }
...
// unwraps HIP wrappers, compressed and uncompressed offload bundles, and raw
// AMDGPU ELF images, so this entry point shares the data-loading path.
...
HIPAPI hipError_t hipModuleUnload(hipModule_t module) {
  ...
  hipError_t remove_result =
      iree_hip_live_module_remove(module, &streaming_module);
...
HIPAPI hipError_t hipModuleGetFunction(hipFunction_t* function,
  ...
  *function = NULL;
  ...
  iree_hal_streaming_module_t* stream_module = NULL;
  hipError_t acquire_result =
      iree_hip_live_module_acquire(module, &stream_module);
  if (acquire_result != hipSuccess) {
    HIP_RETURN_ERROR(module ? hipErrorInvalidHandle : acquire_result);
  }
...
//  - hipErrorInvalidValue: dptr and bytes are both NULL, or name is empty.
//  - hipErrorInvalidResourceHandle: hmod is NULL.
...
HIPAPI hipError_t hipModuleGetGlobal(hipDeviceptr_t* dptr, size_t* bytes,
  ...
  hipError_t acquire_result = iree_hip_live_module_acquire(hmod, &module);
  if (acquire_result != hipSuccess) {
    HIP_RETURN_ERROR(acquire_result);
  }
  ...
  if (iree_status_is_ok(status)) {
    if (dptr) *dptr = (hipDeviceptr_t)device_ptr;
...
HIPAPI hipError_t hipModuleGetFunctionCount(unsigned int* count,
                                            hipModule_t module) {
  if (!count) HIP_RETURN_ERROR(hipErrorInvalidValue);

  iree_hal_streaming_module_t* stream_module = NULL;
  hipError_t acquire_result =
      iree_hip_live_module_acquire(module, &stream_module);
  if (acquire_result != hipSuccess) HIP_RETURN_ERROR(acquire_result);

  iree_host_size_t function_count = 0;
  for (iree_host_size_t i = 0; i < stream_module->symbol_count; ++i) {
    if (stream_module->symbols[i].type ==
        IREE_HAL_STREAMING_SYMBOL_TYPE_FUNCTION) {
      ++function_count;
    }
  }
  if (function_count > UINT_MAX) {
    iree_hal_streaming_module_release(stream_module);
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  *count = (unsigned int)function_count;
  iree_hal_streaming_module_release(stream_module);
  return hipSuccess;
}
...
HIPAPI hipError_t hipGetFuncBySymbol(hipFunction_t* functionPtr,
  ...
  if (!functionPtr || !symbolPtr) HIP_RETURN_ERROR(hipErrorInvalidValue);
  *functionPtr = NULL;
  ...
  iree_status_t status = iree_hal_streaming_context_symbol_map_lookup(
      &context->symbol_map, (void*)symbolPtr, &symbol);
  if (!iree_status_is_ok(status)) {
    iree_status_ignore(status);
    HIP_RETURN_ERROR(hipErrorInvalidDeviceFunction);
  }
  if (symbol == (iree_hal_streaming_symbol_t*)symbolPtr ||
      symbol->type != IREE_HAL_STREAMING_SYMBOL_TYPE_FUNCTION) {
    HIP_RETURN_ERROR(hipErrorInvalidDeviceFunction);
  }

  *functionPtr = (hipFunction_t)iree_hal_streaming_symbol_tag(symbol);
  return hipSuccess;
}
...
static hipError_t iree_hip_resolve_function_for_metadata(
    const void* function_address, hipError_t null_result,
    hipError_t invalid_result, iree_hal_streaming_symbol_t** out_symbol) {
  *out_symbol = NULL;
  if (!function_address) return null_result;

  iree_hal_streaming_context_t* context = NULL;
  hipError_t result = iree_hip_ensure_context(&context);
  if (result != hipSuccess) return result;
  result =
      iree_hip_resolve_function_symbol(context, function_address, out_symbol);
  return result == hipSuccess ? hipSuccess : invalid_result;
}

static hipError_t iree_hip_function_device(
    const iree_hal_streaming_symbol_t* symbol,
    iree_hal_streaming_device_t** out_device) {
  *out_device = NULL;
  if (!symbol || !symbol->module || !symbol->module->context ||
      !symbol->module->context->device_entry) {
    return hipErrorInvalidDevice;
  }
  *out_device = symbol->module->context->device_entry;
  return hipSuccess;
}

static hipError_t iree_hip_function_binary_version(
    const iree_hal_streaming_symbol_t* symbol, int* out_version) {
  iree_hal_streaming_device_t* device = NULL;
  hipError_t result = iree_hip_function_device(symbol, &device);
  if (result != hipSuccess) return result;
  const uint32_t major = device->compute_capability_major;
  const uint32_t minor = device->compute_capability_minor;
  const uint64_t version = (uint64_t)major * 10 + minor;
  if (version > INT_MAX) return hipErrorInvalidValue;
  *out_version = (int)version;
  return hipSuccess;
}

static hipError_t iree_hip_function_attribute(
    const iree_hal_streaming_symbol_t* symbol, hipFuncAttribute_t attribute,
    int* out_value) {
  ...
  switch (attribute) {
    case hipFuncAttributeMaxThreadsPerBlock:
      *out_value = (int)attributes->maximum_threads_per_block;
      return hipSuccess;
    case hipFuncAttributeSharedSizeBytes:
      *out_value = (int)attributes->fixed_shared_memory_size;
      return hipSuccess;
    case hipFuncAttributeConstSizeBytes:
      *out_value = 0;
      return hipSuccess;
    case hipFuncAttributeLocalSizeBytes:
      *out_value = (int)attributes->fixed_local_memory_size;
      return hipSuccess;
    case hipFuncAttributeNumRegs:
      *out_value = (int)attributes->register_count;
      return hipSuccess;
    case hipFuncAttributePtxVersion:
    case hipFuncAttributeBinaryVersion:
      return iree_hip_function_binary_version(symbol, out_value);
...
//  - function_address: [IN] Registered host function or tagged module handle.
//
// Returns:
...
HIPAPI hipError_t hipFuncGetAttribute(int* pi, hipFuncAttribute_t attrib,
  ...
  hipError_t result = iree_hip_resolve_function_for_metadata(
      hfunc, hipErrorInvalidResourceHandle, hipErrorInvalidResourceHandle,
      &symbol);
  if (result != hipSuccess) HIP_RETURN_ERROR(result);
  HIP_RETURN_ERROR(iree_hip_function_attribute(symbol, attrib, pi));
}
...
//  - hipErrorInvalidValue: attr is NULL.
//  - hipErrorInvalidDeviceFunction: Not a valid kernel function.
//
// Synchronization: This operation is synchronous and immediate.
//
...
HIPAPI hipError_t hipFuncGetAttributes(hipFuncAttributes* attr,
  ...
  iree_hal_streaming_symbol_t* symbol = NULL;
  hipError_t result = iree_hip_resolve_function_for_metadata(
      function_address, hipErrorInvalidDeviceFunction,
      hipErrorInvalidDeviceFunction, &symbol);
  if (result != hipSuccess) HIP_RETURN_ERROR(result);
  const iree_hal_streaming_function_attributes_t* attributes =
      &symbol->function_attributes;

  int binary_version = 0;
  result = iree_hip_function_binary_version(symbol, &binary_version);
  if (result != hipSuccess) HIP_RETURN_ERROR(result);
  ...
  attr->ptxVersion = binary_version;
  attr->binaryVersion = binary_version;
  attr->cacheModeCA = 0;
...
static hipError_t iree_hip_launch_kernel(const void* function_address,
  ...
  if (start_event) {
    hipError_t event_result = hipEventRecord(start_event, stream);
    if (event_result != hipSuccess) {
      iree_hip_resolved_stream_release(&resolved_stream);
      IREE_TRACE_ZONE_END(z0);
      HIP_RETURN_ERROR(event_result);
  ...
  if (result == hipSuccess && stop_event) {
    result = hipEventRecord(stop_event, stream);
  }
...
HIPAPI hipError_t hipExtLaunchMultiKernelMultiDevice(
  ...
  if ((iree_host_size_t)numDevices > available_device_count) {
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  ...
  for (int i = 0; i < numDevices && result == hipSuccess; ++i) {
    ...
    if (i > 0) {
      const hipLaunchParams* previous = &launchParamsList[i - 1];
      if (launch->func != previous->func ||
          launch->gridDim.x != previous->gridDim.x ||
          launch->gridDim.y != previous->gridDim.y ||
          launch->gridDim.z != previous->gridDim.z ||
          launch->blockDim.x != previous->blockDim.x ||
          launch->blockDim.y != previous->blockDim.y ||
          launch->blockDim.z != previous->blockDim.z ||
          launch->sharedMem != previous->sharedMem) {
        result = hipErrorInvalidValue;
        break;
      }
    }
    ...
    result = iree_hip_resolve_function_symbol(context, launch->func,
                                              &launches[i].symbol);
    if (result != hipSuccess) break;
    result = iree_hip_validate_launch_configuration(
        context->device_entry, launches[i].symbol, launch->gridDim.x,
        launch->gridDim.y, launch->gridDim.z, launch->blockDim.x,
        launch->blockDim.y, launch->blockDim.z, launch->sharedMem);
    if (result != hipSuccess) break;

    launches[i].params = (iree_hal_streaming_dispatch_params_t){
        .grid_dim = {launch->gridDim.x, launch->gridDim.y, launch->gridDim.z},
  ...
  for (int i = 0; i < numDevices && result == hipSuccess; ++i) {
    result = iree_hip_order_legacy_stream_dependencies(
        launches[i].stream->context, launches[i].stream);
  }
  if (result == hipSuccess &&
  ...
  if (result == hipSuccess) {
    result = iree_status_to_hip_result(iree_hal_streaming_launch_kernel_batch(
        (iree_host_size_t)numDevices, launches));
  }
  if ((flags & hipCooperativeLaunchMultiDeviceNoPostSync) == 0 &&
      retained_stream_count == (iree_host_size_t)numDevices) {
...
static hipError_t iree_hip_module_launch_kernel(
    ...
    void** kernelParams, void** extra, hipEvent_t start_event,
    hipEvent_t stop_event) {
  IREE_TRACE_ZONE_BEGIN(z0);
  HIP_DEBUG_LOG(
      "[HIP_API] hipModuleLaunchKernel: f=%p grid=(%u,%u,%u) "
      "block=(%u,%u,%u) shared=%u stream=%p extra=%p kernelParams=%p\n",
      (void*)f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ,
      sharedMemBytes, (void*)stream, (void*)extra, (void*)kernelParams);
  ...
  if (!iree_status_is_ok(status)) {
    ...
    iree_status_format(status, sizeof(buf), buf, &buf_len);
    fprintf(stderr,
            "[HRX] hipModuleLaunchKernel FAILED symbol=%p buffer=%p "
            "buffer_size=%zu grid=(%u,%u,%u) block=(%u,%u,%u): %.*s\n",
            (void*)symbol, params_ptr, params_size, gridDimX, gridDimY,
            gridDimZ, blockDimX, blockDimY, blockDimZ, (int)buf_len, buf);
    fflush(stderr);
  }
...
//
// Workgroups smaller than the requested local size use the global size for
// that dimension. This path cannot represent a partial final workgroup; when
// executable metadata permits one, reject the launch instead of dispatching
// extra work-items beyond the requested global size.
HIPAPI hipError_t hipExtModuleLaunchKernel(
  ...
  if (sharedMemBytes > UINT32_MAX) HIP_RETURN_ERROR(hipErrorInvalidValue);
  if (!iree_hal_streaming_symbol_has_tag(f)) {
    HIP_RETURN_ERROR(hipErrorInvalidHandle);
  }
  iree_hal_streaming_symbol_t* symbol = iree_hal_streaming_symbol_untag(f);
  ...
  iree_hal_streaming_context_t* module_context =
      symbol->module ? symbol->module->context : NULL;
  hipError_t block_config_result = iree_hip_validate_launch_block_configuration(
      module_context ? module_context->device_entry : NULL, symbol,
      localWorkSizeX, localWorkSizeY, localWorkSizeZ, sharedMemBytes);
  if (block_config_result != hipSuccess) {
    HIP_RETURN_ERROR(block_config_result);
  }

  // A valid local shape may be larger than a small global dimension. Clamp
  // only after validating the requested shape so an oversized workgroup does
  // not become valid merely because the global work size is small.
  localWorkSizeX = iree_min(globalWorkSizeX, localWorkSizeX);
  localWorkSizeY = iree_min(globalWorkSizeY, localWorkSizeY);
  localWorkSizeZ = iree_min(globalWorkSizeZ, localWorkSizeZ);
  const bool has_partial_workgroup = (globalWorkSizeX % localWorkSizeX) ||
                                     (globalWorkSizeY % localWorkSizeY) ||
                                     (globalWorkSizeZ % localWorkSizeZ);
  if (has_partial_workgroup) HIP_RETURN_ERROR(hipErrorNotSupported);

  const unsigned int gridDimX = globalWorkSizeX / localWorkSizeX;
  const unsigned int gridDimY = globalWorkSizeY / localWorkSizeY;
  const unsigned int gridDimZ = globalWorkSizeZ / localWorkSizeZ;
  return iree_hip_module_launch_kernel(
      f, gridDimX, gridDimY, gridDimZ, localWorkSizeX, localWorkSizeY,
      localWorkSizeZ, (unsigned int)sharedMemBytes, stream, kernelParams, extra,
...
static hipError_t iree_hip_user_object_release_refs_internal(
  ...
  if (release_graph_association && user_object->graph_association_count == 0) {
    ...
    // Releasing more references than are currently owned succeeds without
    // changing the payload reference count. Graph association cleanup remains
    // independent so an over-moved reference count cannot retain the control
    // block after its graph has been destroyed.
...
// Returns true only while |graph| remains a public HIP graph handle owned by
// its caller. Executable graphs may retain their source after the public graph
// handle has been destroyed, so object lifetime alone is insufficient here.
static bool iree_hip_live_graph_contains(hipGraph_t graph) {
  if (!graph) return false;
  iree_hip_live_graph_lock();
  for (iree_hip_live_graph_entry_t* current = iree_hip_live_graph_head; current;
       current = current->next) {
...
typedef bool (*iree_hip_graph_tree_visit_fn_t)(
    iree_hal_streaming_graph_t* graph, void* user_data);

static bool iree_hip_graph_tree_append(iree_hal_streaming_graph_t*** graphs,
                                       size_t* graph_count,
  ...
  if (*graph_count == *graph_capacity) {
    size_t new_capacity = *graph_capacity ? *graph_capacity * 2 : 8;
    if (new_capacity < *graph_capacity ||
        new_capacity > SIZE_MAX / sizeof(**graphs)) {
      return false;
    }
    void* new_graphs = realloc(*graphs, new_capacity * sizeof(**graphs));
    if (!new_graphs) return false;
    *graphs = (iree_hal_streaming_graph_t**)new_graphs;
...
static bool iree_hip_graph_tree_append_children(
    iree_hal_streaming_graph_t* graph, iree_hal_streaming_graph_t*** graphs,
    size_t* graph_count, size_t* graph_capacity) {
  for (iree_hal_streaming_node_block_t* block = graph->node_blocks; block;
       block = block->next) {
    for (iree_host_size_t node_index = 0; node_index < block->count;
         ++node_index) {
      iree_hal_streaming_graph_node_t* node = block->nodes[node_index];
      if (node->type != IREE_HAL_STREAMING_GRAPH_NODE_TYPE_GRAPH ||
          !node->attrs.child_graph.graph) {
        continue;
      }
      if (!iree_hip_graph_tree_append(graphs, graph_count, graph_capacity,
                                      node->attrs.child_graph.graph)) {
        return false;
      }
    }
...
// Visits graphs reachable through child graph nodes. Holding the public graph
// registry lock keeps each root alive while its owned child references are
// traversed. Graph mutation remains externally synchronized by the HIP API.
static bool iree_hip_visit_graph_tree_locked(
    iree_hal_streaming_graph_t* root, iree_hip_graph_tree_visit_fn_t visit,
    void* user_data) {
  iree_hal_streaming_graph_t** graphs = NULL;
  size_t graph_count = 0;
  size_t graph_capacity = 0;
  if (visit(root, user_data)) return true;
  if (!iree_hip_graph_tree_append_children(root, &graphs, &graph_count,
                                           &graph_capacity)) {
    free(graphs);
    return false;
  }
  ...
  bool found = false;
  for (size_t graph_index = 0; graph_index < graph_count && !found;
       ++graph_index) {
    iree_hal_streaming_graph_t* graph = graphs[graph_index];
    if (visit(graph, user_data)) {
      found = true;
      break;
    }
    if (!iree_hip_graph_tree_append_children(graph, &graphs, &graph_count,
                                             &graph_capacity)) {
      free(graphs);
      return false;
    }
...
static bool iree_hip_graph_tree_matches_graph(iree_hal_streaming_graph_t* graph,
                                              void* user_data) {
...
static bool iree_hip_live_graph_contains_reachable(hipGraph_t graph) {
  if (!graph) return false;
  iree_hip_live_graph_lock();
  bool found = false;
  for (iree_hip_live_graph_entry_t* entry = iree_hip_live_graph_head;
    ...
    found = iree_hip_visit_graph_tree_locked(
        ...
        iree_hip_graph_tree_matches_graph, graph);
  }
  iree_slim_mutex_unlock(&iree_hip_live_graph_mutex);
  return found;
}
...
static bool iree_hip_graph_handle_is_live(hipGraph_t graph) {
  return iree_hal_streaming_graph_is_capture_active(
             (const iree_hal_streaming_graph_t*)graph) ||
         iree_hip_live_graph_contains(graph) ||
         iree_hip_live_graph_contains_reachable(graph);
}
...
HIPAPI hipError_t hipGraphLaunch(hipGraphExec_t graphExec, hipStream_t stream) {
  ...
  bool requires_host_completion = false;
  iree_status_t launch_status = iree_hal_streaming_graph_exec_launch(
      exec, resolved_stream.stream, &requires_host_completion);
  if (iree_status_is_ok(launch_status) && requires_host_completion) {
    launch_status =
        iree_hal_streaming_stream_synchronize(resolved_stream.stream);
  }
  iree_hal_streaming_graph_exec_release(exec);
  ...
  if (!iree_status_is_ok(launch_status)) {
    const hipError_t fallback_result =
        iree_status_code(launch_status) == IREE_STATUS_UNIMPLEMENTED
            ? hipErrorNotSupported
            : hipErrorInvalidValue;
    hipError_t result =
        iree_status_to_fixed_hip_result(launch_status, fallback_result);
    IREE_TRACE_ZONE_END(z0);
    HIP_RETURN_ERROR(result);
  }
...
static void iree_hip_graph_set_pageable_d2h_completion(
    iree_hal_streaming_graph_node_t* callback_node,
    iree_hal_streaming_context_t* context, const void* dst,
    iree_host_size_t size) {
  callback_node->flags &=
      ~IREE_HAL_STREAMING_GRAPH_NODE_FLAG_REQUIRES_HOST_COMPLETION;
  if (iree_hip_memcpy_destination_is_pageable(context, dst, size)) {
    callback_node->flags |=
        IREE_HAL_STREAMING_GRAPH_NODE_FLAG_REQUIRES_HOST_COMPLETION;
  }
}
...
// Resolves a graph device endpoint to the allocation that owns it. A graph may
// describe work on peer allocations, so its execution context is not
// necessarily the allocation owner. Executable graph resource retention keeps
// the resolved HAL buffer alive; device access policy remains responsible for
// making the underlying address accessible when the graph is launched.
static iree_status_t iree_hip_graph_resolve_device_buffer_ref(
    iree_hal_streaming_graph_t* graph, const void* ptr, iree_device_size_t size,
    iree_hal_streaming_buffer_ref_t* out_ref) {
  iree_status_t status = iree_hal_streaming_memory_lookup_range(
      graph->context, (iree_hal_streaming_deviceptr_t)ptr, size, out_ref);
  if (iree_status_is_ok(status) ||
      iree_status_code(status) != IREE_STATUS_NOT_FOUND) {
    return status;
  }
  ...
  iree_hal_streaming_context_t* owner_context = NULL;
  status = iree_hal_streaming_memory_lookup_range_across_contexts(
      (iree_hal_streaming_deviceptr_t)ptr, size, &owner_context, out_ref);
  if (iree_status_is_ok(status)) {
    iree_hal_streaming_context_release(owner_context);
  }
  return status;
...
HIPAPI hipError_t hipGraphAddMemcpyNode(hipGraphNode_t* pGraphNode,
  ...
  if (!iree_status_is_ok(status)) {
    ...
    callback_node->flags |= IREE_HAL_STREAMING_GRAPH_NODE_FLAG_HIDDEN;
    callback_node->attrs.host.user_data_size = sizeof(*callback_data);
    iree_hip_graph_set_pageable_d2h_completion(
        callback_node, stream_graph->context, dst, size);
...
HIPAPI hipError_t hipGraphAddMemcpyNode1D(hipGraphNode_t* pGraphNode,
  ...
  if (kind == hipMemcpyDeviceToDevice) {
    ...
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
        z0,
        iree_hip_graph_resolve_device_buffer_ref(stream_graph, dst, count,
                                                 &dst_ref),
        hipErrorInvalidValue);
    iree_hal_streaming_buffer_t* staging = NULL;
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
    ...
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
        ...
        iree_hal_streaming_memory_lookup_range(
            stream_graph->context, staging->device_ptr, count, &staging_ref),
        hipErrorInvalidValue);
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
        z0,
        iree_hal_streaming_graph_add_copy_buffer_node_with_extra_dependency(
            stream_graph, deps, numDependencies, callback_node, dst_ref,
            staging_ref, count, &node),
        hipErrorInvalidValue);
    iree_hip_graph_store_1d_memcpy_metadata(node, dst, src, count,
                                            requested_kind);
    *pGraphNode = (hipGraphNode_t)node;
    ...
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
        z0,
        iree_hip_graph_resolve_device_buffer_ref(stream_graph, src, count,
                                                 &src_ref),
        hipErrorInvalidValue);
    iree_hal_streaming_buffer_t* staging = NULL;
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
    ...
    HIP_RETURN_STATUS_AND_END_ZONE_IF_ERROR(
        z0,
        iree_hal_streaming_graph_add_copy_buffer_node(
            stream_graph, deps, numDependencies, staging_ref, src_ref, count,
            &copy_node),
        hipErrorInvalidValue);
    ...
    node->attrs.host.user_data_size = sizeof(*callback_data);
    iree_hip_graph_set_pageable_d2h_completion(node, stream_graph->context, dst,
                                               count);
...
HIPAPI hipError_t hipGraphDestroyNode(hipGraphNode_t node) {
  if (!iree_hal_streaming_graph_is_live_node(
          (iree_hal_streaming_graph_node_t*)node)) {
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
...
HIPAPI hipError_t hipGraphClone(hipGraph_t* pGraphClone,
  ...
  *pGraphClone = NULL;
  ...
  hipError_t context_result = iree_hip_ensure_context(&target_context);
  if (context_result != hipSuccess) {
    HIP_RETURN_ERROR(context_result);
  }
  iree_hal_streaming_graph_t* clone_graph = NULL;
  // Clone-owned staging is created in the caller's current context. Explicit
  // node affinity and the source graph's execution hint independently select
  // where the executable is compiled.
  iree_status_t clone_status = iree_hal_streaming_graph_clone(
      (iree_hal_streaming_graph_t*)originalGraph, target_context, &clone_graph);
  if (!iree_status_is_ok(clone_status)) {
    const hipError_t result =
        iree_status_code(clone_status) == IREE_STATUS_UNIMPLEMENTED
            ? hipErrorNotSupported
            : iree_status_to_fixed_hip_result(clone_status,
                                              hipErrorInvalidValue);
    return result;
  }
...
static const char* iree_hip_graph_debug_memcpy_direction(
  ...
  const bool dst_array_like = dst_array || node->attrs.memcpy.hip_dst_symbol;
  switch ((hipMemcpyKind)node->attrs.memcpy.hip_kind) {
    case hipMemcpyHostToHost:
      return "HtoH";
    case hipMemcpyHostToDevice:
      return dst_array_like ? "HtoA" : "HtoD";
    case hipMemcpyDeviceToHost:
      return src_array ? "AtoH" : "DtoH";
    case hipMemcpyDeviceToDevice:
      if (src_array && dst_array_like) return "AtoA";
      if (src_array) return "AtoD";
      if (dst_array_like) return "DtoA";
      return "DtoD";
...
HIPAPI hipError_t hipGraphAddMemcpyNodeFromSymbol(
  ...
  return hipGraphAddMemcpyNode1D(pGraphNode, graph, pDependencies,
                                 numDependencies, dst, symbol_ptr, count, kind);
}
...
HIPAPI hipError_t hipGraphAddMemcpyNodeToSymbol(
  ...
  result =
      hipGraphAddMemcpyNode1D(pGraphNode, graph, pDependencies, numDependencies,
                              symbol_ptr, src, count, kind);
  if (result == hipSuccess) {
    iree_hal_streaming_graph_node_t* node =
        (iree_hal_streaming_graph_node_t*)*pGraphNode;
    node->attrs.memcpy.hip_dst_symbol = true;
  }
  return result;
}

// Adds an allocation node after the caller has established graph ownership.
// Public APIs validate external handles; stream capture can pass its private
// graph directly without publishing it in the public graph registry.
static hipError_t iree_hip_graph_add_mem_alloc_node(
    hipGraphNode_t* pGraphNode, iree_hal_streaming_graph_t* stream_graph,
    const hipGraphNode_t* pDependencies, size_t numDependencies,
    hipMemAllocNodeParams* params) {
  if (!pGraphNode || !stream_graph || !params ||
      (numDependencies > 0 && !pDependencies)) {
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  *pGraphNode = NULL;
  ...
  iree_status_t allocation_status =
      iree_hal_streaming_graph_memory_allocation_create(
          stream_graph->context, params->bytesize, &allocation);
  if (!iree_status_is_ok(allocation_status)) {
    iree_status_ignore(iree_hal_streaming_graph_destroy_node(node));
    if (iree_status_code(allocation_status) == IREE_STATUS_UNAVAILABLE) {
      iree_status_free(allocation_status);
      return hipErrorNotSupported;
    }
    return iree_status_to_fixed_hip_result(allocation_status,
                                           hipErrorOutOfMemory);
  }
  params->dptr =
...
HIPAPI hipError_t hipGraphAddMemAllocNode(hipGraphNode_t* pGraphNode,
                                          ...
                                          size_t numDependencies,
                                          void* allocParams) {
  if (!graph || !iree_hip_graph_handle_is_live(graph)) {
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  HIP_RETURN_ERROR(iree_hip_graph_add_mem_alloc_node(
      pGraphNode, (iree_hal_streaming_graph_t*)graph, pDependencies,
      numDependencies, (hipMemAllocNodeParams*)allocParams));
}

// Adds a free node to a graph whose ownership has already been established.
// Public graph APIs validate a caller-owned handle before using this helper;
// stream capture passes its private capture graph directly.
static hipError_t iree_hip_graph_add_mem_free_node(
    hipGraphNode_t* pGraphNode, iree_hal_streaming_graph_t* stream_graph,
    const hipGraphNode_t* pDependencies, size_t numDependencies, void* dptr) {
  if (!pGraphNode || !stream_graph || !dptr ||
      (numDependencies > 0 && !pDependencies)) {
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  *pGraphNode = NULL;
  ...
  iree_status_t lookup_status =
      iree_hal_streaming_graph_memory_allocation_lookup(
          stream_graph->context, (uint64_t)(uintptr_t)dptr, &allocation);
  if (!iree_status_is_ok(lookup_status)) {
  ...
  iree_hal_streaming_graph_node_t* node = NULL;
  iree_status_t add_node_status = iree_hal_streaming_graph_add_empty_node(
      stream_graph, (iree_hal_streaming_graph_node_t**)pDependencies,
      numDependencies, &node);
  if (!iree_status_is_ok(add_node_status)) {
    iree_hal_streaming_graph_memory_allocation_release_free_node_reference(
        allocation);
    iree_hal_streaming_graph_memory_allocation_release(allocation);
    hipError_t result =
        iree_status_to_fixed_hip_result(add_node_status, hipErrorInvalidValue);
    return result;
  }
...
HIPAPI hipError_t hipGraphAddMemFreeNode(hipGraphNode_t* pGraphNode,
  ...
  HIP_RETURN_ERROR(iree_hip_graph_add_mem_free_node(
      pGraphNode, (iree_hal_streaming_graph_t*)graph, pDependencies,
...
static bool iree_hip_graph_batch_wait_flags_are_valid(unsigned int flags) {
  return flags <= HRX_HIP_STREAM_WAIT_VALUE_NOR;
}

static bool iree_hip_graph_batch_write_flags_are_valid(unsigned int flags) {
  return flags == HRX_HIP_STREAM_WRITE_VALUE_DEFAULT ||
         flags == HRX_HIP_STREAM_WRITE_VALUE_INCREMENT ||
         flags == HRX_HIP_STREAM_WRITE_VALUE_DECREMENT;
}
...
static hipError_t iree_hip_graph_validate_batch_mem_op_node_params(
  ...
  if (!params || !params->ctx || params->count == 0 || params->count > 256 ||
      !params->paramArray || params->flags != 0) {
    return hipErrorInvalidValue;
  }
  for (unsigned int i = 0; i < params->count; ++i) {
    const hrx_hip_stream_batch_mem_op_params_t* operation =
        &params->paramArray[i];
    const void* address = NULL;
    iree_host_size_t byte_length = 0;
    switch (operation->operation) {
      case HRX_HIP_STREAM_MEM_OP_WAIT_VALUE_32:
        address = operation->waitValue.address;
        byte_length = sizeof(uint32_t);
        if (!iree_hip_graph_batch_wait_flags_are_valid(
                operation->waitValue.flags)) {
          return hipErrorInvalidValue;
        }
        break;
      case HRX_HIP_STREAM_MEM_OP_WRITE_VALUE_32:
        address = operation->writeValue.address;
        byte_length = sizeof(uint32_t);
        if (!iree_hip_graph_batch_write_flags_are_valid(
                operation->writeValue.flags)) {
          return hipErrorInvalidValue;
        }
        break;
      case HRX_HIP_STREAM_MEM_OP_WAIT_VALUE_64:
        address = operation->waitValue.address;
        ...
        break;
      case HRX_HIP_STREAM_MEM_OP_WRITE_VALUE_64:
        address = operation->writeValue.address;
        byte_length = sizeof(uint64_t);
        if (!iree_hip_graph_batch_write_flags_are_valid(
                operation->writeValue.flags)) {
        ...
        break;
      case HRX_HIP_STREAM_MEM_OP_FLUSH_REMOTE_WRITES:
      case HRX_HIP_STREAM_MEM_OP_BARRIER:
        return hipErrorNotSupported;
      default:
        return hipErrorInvalidValue;
    }
    if (!address || (uintptr_t)address % byte_length != 0) {
      return hipErrorInvalidValue;
    }
  }
...
HIPAPI hipError_t hipGraphAddBatchMemOpNode(hipGraphNode_t* pGraphNode,
  ...
  *pGraphNode = NULL;
  ...
  iree_hal_streaming_graph_node_t* stream_node = NULL;
  HIP_RETURN_STATUS(iree_hal_streaming_graph_add_batch_mem_op_node(
                        (iree_hal_streaming_graph_t*)graph,
                        (iree_hal_streaming_graph_node_t**)pDependencies,
                        numDependencies, params, sizeof(*params),
                        params->paramArray, param_array_size, &stream_node),
                    hipErrorInvalidValue);
  *pGraphNode = (hipGraphNode_t)stream_node;
  return hipSuccess;
}
...
static hipError_t iree_hip_graph_update_memcpy_metadata(
  ...
  iree_hal_streaming_buffer_ref_t dst_ref;
  iree_status_t status = iree_hip_graph_resolve_device_buffer_ref(
      node->graph, dst, dst_span, &dst_ref);
  if (!iree_status_is_ok(status)) {
    iree_status_ignore(status);
    return hipErrorInvalidValue;
  }
  iree_hal_streaming_buffer_ref_t src_ref;
  status = iree_hip_graph_resolve_device_buffer_ref(node->graph, src, src_span,
                                                    &src_ref);
  if (!iree_status_is_ok(status)) {
    iree_status_ignore(status);
    return hipErrorInvalidValue;
  ...
  status = iree_hal_streaming_graph_memcpy_node_set_buffer_refs(node, dst_ref,
                                                                src_ref);
  if (!iree_status_is_ok(status)) {
    iree_status_ignore(status);
    return hipErrorInvalidValue;
...
static void iree_hip_graph_rollback_memcpy_node_update(
  ...
  if (update->pre_callback_node) {
    update->pre_callback_node->flags = update->old_pre_callback_flags;
    iree_hip_graph_memcpy_callback_data_t* callback_data =
        (iree_hip_graph_memcpy_callback_data_t*)
  ...
  if (update->post_callback_node) {
    update->post_callback_node->flags = update->old_post_callback_flags;
    iree_hip_graph_memcpy_callback_data_t* callback_data =
        (iree_hip_graph_memcpy_callback_data_t*)
...
static hipError_t iree_hip_graph_set_same_context_device_memcpy_node_params(
  ...
  if (pre_callback) {
    ...
    update->old_pre_callback_flags = pre_callback->flags;
  ...
  if (post_callback) {
    ...
    update->old_post_callback_flags = post_callback->flags;
...
static hipError_t iree_hip_graph_set_memcpy_node_params(
  ...
  if (kind == hipMemcpyDeviceToDevice) {
    ...
    result = iree_hip_graph_configure_memcpy_callback_data(
        ...
        dst_rows_per_slice, execution_params.extent.height);
    if (result != hipSuccess) return result;
    iree_hip_graph_set_pageable_d2h_completion(post_callback,
                                               node->graph->context, dst, size);
    callback_data->hip_params = *params;
    callback_data->hip_params.kind =
        iree_hip_graph_metadata_memcpy_kind(params->kind);
...
HIPAPI hipError_t hipGraphExecBatchMemOpNodeSetParams(hipGraphExec_t graphExec,
  ...
  iree_hal_streaming_graph_node_t* exec_node = NULL;
  iree_status_t status = iree_hal_streaming_graph_exec_begin_node_update(
      exec, stream_node, &exec_node);
  if (!iree_status_is_ok(status)) {
    iree_hal_streaming_graph_exec_release(exec);
    HIP_RETURN_STATUS(status, hipErrorInvalidValue);
  }
  const iree_hal_streaming_graph_batch_mem_op_node_attrs_t old_attrs =
      exec_node->attrs.batch_mem_op;
  void* old_params = NULL;
  void* old_param_array = NULL;
  if (old_attrs.params_size > 0) {
    status = iree_allocator_malloc(exec_node->graph->host_allocator,
                                   old_attrs.params_size, &old_params);
    if (iree_status_is_ok(status)) {
  ...
  if (iree_status_is_ok(status) && old_attrs.param_array_size > 0) {
    status =
        iree_allocator_malloc(exec_node->graph->host_allocator,
                              old_attrs.param_array_size, &old_param_array);
    if (iree_status_is_ok(status)) {
  ...
  if (!iree_status_is_ok(status)) {
    iree_allocator_free(exec_node->graph->host_allocator, old_params);
    iree_hal_streaming_graph_exec_end_node_update(exec);
    iree_hal_streaming_graph_exec_release(exec);
    HIP_RETURN_STATUS(status, hipErrorOutOfMemory);
  }
  status = iree_hal_streaming_graph_set_batch_mem_op_node_params(
      exec_node, params, sizeof(*params), params->paramArray, param_array_size);
  if (iree_status_is_ok(status)) {
    status = iree_hal_streaming_graph_exec_rebuild_node_update(exec);
  }
  if (!iree_status_is_ok(status)) {
    exec_node->attrs.batch_mem_op = old_attrs;
    if (old_attrs.params_size > 0) {
      memcpy(old_attrs.params, old_params, old_attrs.params_size);
    }
    if (old_attrs.param_array_size > 0) {
      memcpy(old_attrs.param_array, old_param_array,
             old_attrs.param_array_size);
    }
  }
  iree_allocator_free(exec_node->graph->host_allocator, old_param_array);
  iree_allocator_free(exec_node->graph->host_allocator, old_params);
  iree_hal_streaming_graph_exec_end_node_update(exec);
  hipError_t result =
      iree_status_to_fixed_hip_result(status, hipErrorInvalidValue);
...
HIPAPI hipError_t hipGraphExecHostNodeSetParams(hipGraphExec_t graphExec,
  ...
  const iree_hal_streaming_graph_host_call_node_attrs_t old_attrs =
      exec_node->attrs.host;
  exec_node->attrs.host.fn = (void (*)(void*))params->fn;
  exec_node->attrs.host.user_data = params->userData;
  status = iree_hal_streaming_graph_exec_rebuild_node_update(exec);
  if (!iree_status_is_ok(status)) {
    exec_node->attrs.host = old_attrs;
  }
  iree_hal_streaming_graph_exec_end_node_update(exec);
  result = iree_status_to_fixed_hip_result(status, hipErrorInvalidValue);
  iree_hal_streaming_graph_exec_release(exec);
...
HIPAPI hipError_t hipGraphExecMemcpyNodeSetParams(hipGraphExec_t graphExec,
  ...
  if (iree_hip_graph_node_is_visible_memcpy_host_call(stream_node)) {
    ...
    iree_hip_graph_memcpy_callback_data_t* callback_data =
        (iree_hip_graph_memcpy_callback_data_t*)exec_node->attrs.host.user_data;
    ...
    result = iree_hip_graph_set_host_memcpy_callback_params(
        exec_node, params, kind, dst, src, size, &old_callback_data);
    if (result != hipSuccess) {
      iree_hal_streaming_graph_exec_end_node_update(exec);
      iree_hal_streaming_graph_exec_release(exec);
      HIP_RETURN_ERROR(result);
    ...
    status = iree_hal_streaming_graph_exec_rebuild_node_update(exec);
    if (!iree_status_is_ok(status)) {
      iree_hip_graph_release_memcpy_callback_context_refs(exec_node->graph,
                                                          callback_data);
  ...
  const hipMemcpyKind requested_kind =
      iree_hip_graph_metadata_memcpy_kind(params->kind);
  if (requested_kind != exec_node->attrs.memcpy.hip_kind) {
    iree_hal_streaming_graph_exec_end_node_update(exec);
    iree_hal_streaming_graph_exec_release(exec);
    HIP_RETURN_ERROR(hipErrorInvalidValue);
  }
  ...
  status = iree_hal_streaming_graph_exec_rebuild_node_update(exec);
  if (!iree_status_is_ok(status)) {
    iree_hip_graph_rollback_memcpy_node_update(exec_node, &update);
  } else {
    iree_hip_graph_commit_memcpy_node_update(exec_node, &update);
  }
  iree_hal_streaming_graph_exec_end_node_update(exec);
  result = iree_status_to_fixed_hip_result(status, hipErrorInvalidValue);
  iree_hal_streaming_graph_exec_release(exec);
...
HIPAPI hipError_t hipGraphExecMemsetNodeSetParams(hipGraphExec_t graphExec,
  ...
  const bool old_is_2d = exec_node->attrs.memset.hip_height != 1;
  const bool new_is_2d = params->height != 1;
  if (old_is_2d || new_is_2d) {
    if (exec_node->attrs.memset.pattern_size != params->elementSize ||
        exec_node->attrs.memset.count != element_count ||
        exec_node->attrs.memset.hip_width != params->width ||
        exec_node->attrs.memset.hip_height != params->height ||
        exec_node->attrs.memset.hip_pitch != params->pitch) {
      iree_hal_streaming_graph_exec_end_node_update(exec);
      iree_hal_streaming_graph_exec_release(exec);
      HIP_RETURN_ERROR(hipErrorInvalidValue);
    }
...
hipGraphExecKernelNodeSetParams(hipGraphExec_t graphExec, hipGraphNode_t node,
  ...
  iree_hal_streaming_graph_node_t* exec_node = NULL;
  iree_status_t update_status = iree_hal_streaming_graph_exec_begin_node_update(
      exec, stream_node, &exec_node);
  if (!iree_status_is_ok(update_status)) {
    iree_hal_streaming_graph_exec_release(exec);
    HIP_RETURN_STATUS(update_status, hipErrorInvalidValue);
  }
  const iree_hal_streaming_graph_kernel_node_attrs_t old_attrs =
      exec_node->attrs.kernel;
  iree_allocator_t host_allocator = exec_node->graph->host_allocator;
  ...
  result = iree_hip_graph_set_kernel_node_params(
      exec_node, (const hipKernelNodeParams*)pNodeParams);
  if (result != hipSuccess) {
  ...
  update_status = iree_hal_streaming_graph_exec_rebuild_node_update(exec);
  if (!iree_status_is_ok(update_status)) {
    exec_node->attrs.kernel = old_attrs;
    if (old_constants) {
  ...
  iree_allocator_free(host_allocator, old_constants);
  iree_hal_streaming_graph_exec_end_node_update(exec);
  result = iree_status_to_fixed_hip_result(update_status, hipErrorInvalidValue);
  iree_hal_streaming_graph_exec_release(exec);
  return result;
}
...
HIPAPI hipError_t hipGraphExecChildGraphNodeSetParams(hipGraphExec_t graphExec,
  ...
  if (iree_status_is_ok(status)) {
    ...
    exec_node->attrs.child_graph.graph = old_child_graph;
...
HIPAPI hipError_t hipGraphKernelNodeSetParams(hipGraphNode_t node,
  ...
  return iree_hip_graph_set_kernel_node_params(
      stream_node, (const hipKernelNodeParams*)pNodeParams);
}
...
HIPAPI hipError_t hipGraphMemcpyNodeSetParamsFromSymbol(
  ...
  result = hipGraphMemcpyNodeSetParams1D(node, dst, symbol_ptr, count, kind);
  if (result == hipSuccess) {
    iree_hal_streaming_graph_node_t* stream_node =
        (iree_hal_streaming_graph_node_t*)node;
    stream_node->attrs.memcpy.hip_dst_symbol = false;
  }
  return result;
}
...
HIPAPI hipError_t hipGraphMemcpyNodeSetParamsToSymbol(
  ...
  result = hipGraphMemcpyNodeSetParams1D(node, symbol_ptr, src, count, kind);
  if (result == hipSuccess) {
    iree_hal_streaming_graph_node_t* stream_node =
        (iree_hal_streaming_graph_node_t*)node;
    stream_node->attrs.memcpy.hip_dst_symbol = true;
  }
  return result;
}
...
enum {
  IREE_HIP_API_LOOKUP_DEFAULT = 0,
  IREE_HIP_API_LOOKUP_LEGACY_STREAM = 1,
  IREE_HIP_API_LOOKUP_PER_THREAD_DEFAULT_STREAM = 2,
  IREE_HIP_API_LOOKUP_SUCCESS = 0,
  IREE_HIP_API_LOOKUP_SYMBOL_NOT_FOUND = 1,
  IREE_HIP_DEVICE_PROPERTIES_R0600_VERSION = 600,
};

static bool iree_hip_api_lookup_flags_valid(uint64_t flags) {
  return flags == IREE_HIP_API_LOOKUP_DEFAULT ||
         flags == IREE_HIP_API_LOOKUP_LEGACY_STREAM ||
...
static hipError_t iree_hip_lookup_runtime_symbol(const char* symbol,
                                                 ...
                                                 void** function,
                                                 void* symbol_status) {
  if (!symbol || !symbol[0] || !function) return hipErrorInvalidValue;
  ...
  // dlsym(dlopen(NULL), ...) lookup cannot resolve its symbols. Fall back to
  // the global scope only if the self-handle could not be established.
  ...
  const size_t spt_suffix_length = sizeof("_spt") - 1;
  const size_t spt_symbol_extra_length = sizeof("_spt");
  const bool is_spt_symbol =
      symbol_length >= spt_suffix_length &&
      strcmp(symbol + symbol_length - spt_suffix_length, "_spt") == 0;
  if (prefer_spt && !is_spt_symbol) {
    if (symbol_length > SIZE_MAX - spt_symbol_extra_length) {
      if (close_handle) dlclose(handle);
      return hipErrorInvalidValue;
    }
    char* spt_symbol = (char*)malloc(symbol_length + spt_symbol_extra_length);
    if (!spt_symbol) {
      if (close_handle) dlclose(handle);
      return hipErrorOutOfMemory;
    }
    memcpy(spt_symbol, symbol, symbol_length);
    memcpy(spt_symbol + symbol_length, "_spt", spt_symbol_extra_length);
    found = dlsym(handle, spt_symbol);
    free(spt_symbol);
  }
  ...
  *function = found;
  if (symbol_status) {
    *(int*)symbol_status = found ? IREE_HIP_API_LOOKUP_SUCCESS
                                 : IREE_HIP_API_LOOKUP_SYMBOL_NOT_FOUND;
  }
  return found ? hipSuccess : hipErrorInvalidValue;
}

static hipError_t iree_hip_get_proc_address(const char* symbol, void** function,
  ...
  bool prefer_spt = flags == IREE_HIP_API_LOOKUP_PER_THREAD_DEFAULT_STREAM;
  if (is_spt_api && flags == IREE_HIP_API_LOOKUP_DEFAULT) {
    prefer_spt = true;
  }
...
HIPAPI hipError_t hipGetProcAddress(const char* symbol, void** function,
                                    int hip_version, uint64_t flags,
                                    void* symbol_status) {
  return iree_hip_get_proc_address(symbol, function, hip_version, flags,
                                   /*is_spt_api=*/false, symbol_status);
}
...
HIPAPI hipError_t hipGetDriverEntryPoint(const char* symbol, void** function,
  ...
  return iree_hip_lookup_runtime_symbol(
      symbol, flags == IREE_HIP_API_LOOKUP_PER_THREAD_DEFAULT_STREAM, function,
      status);
}
...
HIPAPI hipError_t hipGetDriverEntryPoint_spt(const char* symbol,
  ...
  const bool prefer_spt = flags != IREE_HIP_API_LOOKUP_LEGACY_STREAM;
  return iree_hip_lookup_runtime_symbol(symbol, prefer_spt, function, status);
}

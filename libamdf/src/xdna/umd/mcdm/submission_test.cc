...
TEST(WindowsXdnaSubmissionLayoutTest, BuildsContextLifecycleRecords) {
  const uint32_t header_length = 120;
  ...
  amdf_windows_xdna_submission_build_aperture(&instruction, &aperture);
  ...
  amdf_windows_xdna_submission_build_context_initialize(&command, &initialize);
  ...
  EXPECT_EQ(ReadU32(initialize.bytes, 0x38), 0u);
  EXPECT_EQ(ReadU32(initialize.bytes, 0x3C), 8u);
  EXPECT_EQ(ReadU64(initialize.bytes, 0x40),
  ...
  amdf_windows_xdna_submission_build_accounting(UINT64_C(0x20000), &accounting);
  EXPECT_EQ(accounting.byte_length, header_length);
  EXPECT_EQ(ReadU64(accounting.bytes, 0x00), 9u);
  EXPECT_EQ(ReadU64(accounting.bytes, 0x08), 0u);
  EXPECT_EQ(ReadU64(accounting.bytes, 0x10), UINT64_C(0x20000));
}

TEST(WindowsXdnaSubmissionLayoutTest, BuildsInstructionRangeExecutionRecords) {
  const uint32_t header_length = 120;
  ...
  amdf_windows_xdna_submission_build_execute(&execution, &command, &packet,
                                             &submission);
  ...
  EXPECT_EQ(ReadU32(submission.bytes, 0x38), 8u);
  EXPECT_EQ(ReadU32(submission.bytes, 0x3C), 8u);
  EXPECT_EQ(ReadU64(submission.bytes, 0x40),

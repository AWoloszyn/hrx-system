...
static void amdf_xdna_memory_set_info(amdf_memory_t* memory,
  ...
  memory->accesses[0].info.device_id = amdf_xdna_device_get_info(device)->id;
  ...
  memory->accesses[0].info.address_kinds = result.address_kinds;
  memory->accesses[0].addresses[AMDF_MEMORY_ADDRESS_XDNA_DMA] =
      result.dma_address;
...
amdf_status_t amdf_xdna_memory_prepare(
  ...
  const amdf_status_t status = amdf_xdna_umd_memory_prepare(
      amdf_xdna_device_get_umd(memory->accesses[0].device), profile,
      create_info, &native, &result);
  memory->native = native;
  if (amdf_status_is_ok(status)) {
    amdf_xdna_memory_set_info(memory, memory->accesses[0].device, profile,

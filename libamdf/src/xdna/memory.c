...
static amdf_status_t amdf_xdna_memory_export(
  ...
  return amdf_xdna_umd_memory_export(memory->native, export_info, out_value);
}
...
static amdf_status_t amdf_xdna_memory_describe_site(
  ...
  return amdf_xdna_umd_memory_describe_site(memory->native, &query,
                                            out_description);
}
...
static amdf_status_t amdf_xdna_memory_map(
  ...
  if (amdf_status_is_ok(status)) {
    status = amdf_xdna_umd_memory_map(memory->native, profile, map_info,
                                      &mapping->umd, &result);
  }
  if (amdf_status_is_ok(status)) {
...
static amdf_status_t amdf_xdna_memory_destroy_native(amdf_memory_t* memory) {
  const amdf_status_t status = amdf_xdna_umd_memory_destroy(memory->native);
  if (amdf_status_is_ok(status)) {
    memory->native = NULL;
  }
  return status;
...
static void amdf_xdna_memory_abandon_native(amdf_memory_t* memory) {
  amdf_xdna_umd_memory_abandon(memory->native);
  memory->native = NULL;
}
...
static void amdf_xdna_memory_set_info(amdf_memory_t* memory,
  ...
  memory->info.device_id = amdf_xdna_device_get_info(device)->id;
  ...
  memory->addresses[AMDF_MEMORY_ADDRESS_XDNA_FIRMWARE] = result.device_address;
  memory->info.address_kinds = result.address_kinds;
  memory->addresses[AMDF_MEMORY_ADDRESS_XDNA_DMA] = result.dma_address;
  memory->info.reset_epoch = amdf_xdna_device_query_reset_epoch(device);
}

amdf_status_t amdf_xdna_memory_prepare(
    amdf_memory_t* memory, const amdf_memory_profile_t* profile,
    const amdf_memory_create_info_t* create_info) {
  memory->vtable = &amdf_xdna_memory_vtable;
  amdf_xdna_umd_memory_t* native = NULL;
  amdf_xdna_umd_memory_result_t result = {0};
  const amdf_status_t status =
      amdf_xdna_umd_memory_prepare(amdf_xdna_device_get_umd(memory->device),
                                   profile, create_info, &native, &result);
  memory->native = native;
  if (amdf_status_is_ok(status)) {
    amdf_xdna_memory_set_info(memory, memory->device, profile,
...
amdf_status_t amdf_xdna_memory_prepare_import(
  ...
  const amdf_status_t status = amdf_xdna_umd_memory_prepare_import(
      amdf_xdna_device_get_umd(memory->device), profile, import_info,

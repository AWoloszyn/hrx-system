...
iree-bazel-run --config=asan //experimental/xdna:iree-xdna-run -- \
  ...
  --binding_memory=registered_host \
...
requirements. Each binding has its own native attachment, persistent host view,
and logical HAL buffer. Registered bindings use caller storage aligned to the
image contract and deliberately offset within a native page whenever that
alignment permits. Successful teardown checks that every caller byte remains
accessible after the native attachment is destroyed.

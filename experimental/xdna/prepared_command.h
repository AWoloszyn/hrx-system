...
//
// The caller resolves the HAL buffer and its native backing before constructing
// this record. All fields are borrowed during preparation. Success retains
// |buffer_ref.buffer| until prepared-command destruction. The backing owner
// keeps |memory| alive for that lifetime; libamdf receives only the instruction
// memory range and cannot retain indirectly referenced bindings.
...
// One cold instruction instance retaining its native executable and HAL
// buffers. Native memory, mappings, contexts and queues remain caller-owned.
...
// Releases the retained executable and buffers. The caller has retired every
// use and remains responsible for unmapping and destroying instruction memory.

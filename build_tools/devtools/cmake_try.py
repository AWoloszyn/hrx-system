...
from build_tools.devtools import cmake_cache, cmake_file_api
        ...
        try:
            try:
                ...
                write_initial_cache_file(
                    ...
                    cmake_cache.load_cmake_cache(self.configured_build_dir),
                )
...
def write_initial_cache_file(
    path: Path, entries: list[cmake_cache.CMakeCacheEntry]
) -> None:
    lines = [
...
def should_preserve_cache_entry(entry: cmake_cache.CMakeCacheEntry) -> bool:
...
def write_try_cmake_file(
    path: Path, *, source_paths: list[PurePath], deps: list[str]
) -> None:
    lines = [
    ...
    for source_path in source_paths:
        lines.append(f"  {cmake_bracket_quote(source_path.as_posix())}")
    lines.append(")")
    lines.append(

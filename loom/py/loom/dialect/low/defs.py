...
low_copy = Op(
    ...
    attrs=[
        AttrDef(
            ...
            default=False,
            elide_default=True,
            doc=("Prefer physical storage disjoint from the copy source. This placement preference does not require a materialized copy when allocation reuses the source storage."),
        ),
    ],
...
low_move = Op(
    ...
    attrs=[
        AttrDef(
            ...
            default=False,
            elide_default=True,
            doc=("Prefer physical storage disjoint from the move source. Reusing the source storage is permitted; the source identity is invalid after the move regardless of physical placement."),
        ),
    ],
    results=[Result("result", REGISTER, allocates=True)],
    constraints=[

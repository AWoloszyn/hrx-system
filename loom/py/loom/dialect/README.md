...
The checked-in `ops.h` catalogs operation documentation, types, typed getters,
and builders. Its footer includes `ops.inc`, generated by the same dialect build
action, for named attribute helpers including presence queries and mutation.
Those definitions are available under the corresponding Bazel or CMake output
path.
...
The `set_<field>(module, op, attribute)` helpers use the module's attribute
mutation boundary, maintaining references and derived operation state. During
rewriting, `rewrite_<field>(rewriter, op, attribute)` also maintains facts and
worklists through `loom_rewriter_set_attr`. Both accept a complete
`loom_attribute_t`, including `loom_attr_absent()` to clear optional fields.
Dictionary fields also provide `update_<field>(rewriter, op, updates)` for named
entry changes. Rewriter callers include `loom/rewrite/rewriter.h`. These helpers
are function-like macros that evaluate each argument once.

Construction normally uses the operation's builder. Handwritten construction
uses `initialize_<field>(op, attribute)` before finalization; owner-managed bulk
reference rebuilding can use it before reconstructing reference state. The
initialization caller owns payload lifetime and finalization. Finalized mutation
uses the module or rewriter helpers above.

Shared validators read complete values through `<field>_attr(op)`, preserving
kind and optional absence. This read-only lvalue can be borrowed with `&` when a
shared helper does not need a value copy. `<field>_diagnostic_ref()` identifies
the source field, while `<field>_descriptor(module, op)` returns its schema
metadata. TargetLike operations additionally provide `<field>_from_record(record)`
for detached target-record payloads. Storage ordinals remain inside generated
implementations; generic enumeration and interface metadata retain their indexed
representations.

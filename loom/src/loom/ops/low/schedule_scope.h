...
typedef enum loom_low_schedule_control_kind_e {
  ...
  LOOM_LOW_SCHEDULE_CONTROL_STEP = 2,
...
static inline loom_low_schedule_control_kind_t loom_low_schedule_control_kind(
  ...
  switch (op->kind) {
    ...
    case LOOM_OP_LOW_SCHEDULE_STEP:
      return LOOM_LOW_SCHEDULE_CONTROL_STEP;
...
// A scope ID is the source-order index of its begin control plus one. Zero
// denotes the outer, unconstrained instruction stream. Unreachable CFG blocks
...
typedef struct loom_low_schedule_scopes_t {
  ...
  // Number of reachable begin controls, each defining a distinct scope.

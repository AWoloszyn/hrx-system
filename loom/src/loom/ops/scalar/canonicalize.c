...
static loom_op_t* loom_scalar_defining_op(loom_rewriter_t* rewriter,
  ...
  return def_op && !(def_op->flags & LOOM_OP_FLAG_DEAD) ? def_op : NULL;
}
...
static bool loom_scalar_query_power_of_two_bits(loom_rewriter_t* rewriter,
                                                loom_value_id_t value_id,
                                                uint64_t* out_bits) {
  ...
  return loom_value_facts_as_exact_raw_bits(
             loom_rewriter_value_facts(rewriter, value_id), bit_count,
             out_bits) &&
         *out_bits != 0 && (*out_bits & (*out_bits - 1)) == 0;
}
...
static bool loom_scalar_value_facts_are_exact_float(loom_rewriter_t* rewriter,
                                                    loom_value_id_t value_id,
                                                    double expected_value) {
  double actual_value = 0.0;
  return loom_scalar_query_exact_float(rewriter, value_id, &actual_value) &&
         actual_value == expected_value;
}
...
static float loom_scalar_divide_f32(float lhs, float rhs) { return lhs / rhs; }
static double loom_scalar_divide_f64(double lhs, double rhs) {
  return lhs / rhs;
}
...
static iree_status_t loom_scalar_replace_single_result_with_float_constant(
  ...
  IREE_RETURN_IF_ERROR(loom_rewriter_build_constant(
      rewriter,
      loom_value_facts_exact_float(loom_type_element_type(result_type), value),
...
static iree_status_t loom_scalar_replace_mulf_with_negated_constant(
  ...
  IREE_RETURN_IF_ERROR(loom_rewriter_build_constant(
      rewriter, loom_value_facts_exact_float(scalar_type, -constant_value),
      result_type, op->location, &negated_constant));
...
static iree_status_t loom_scalar_replace_divf_with_reciprocal_mulf(
  ...
  loom_value_facts_eval_float_binary(scalar_type, &one_facts, &divisor_facts,
                                     /*f32_fn=*/loom_scalar_divide_f32,
                                     /*f64_fn=*/loom_scalar_divide_f64,
                                     &reciprocal_facts);

  loom_value_id_t reciprocal = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_rewriter_build_constant(
      rewriter, reciprocal_facts, result_type, op->location, &reciprocal));
...
static bool loom_scalar_match_contractable_mulf(loom_rewriter_t* rewriter,
                                                loom_value_id_t value,
                                                loom_op_t** out_product_op) {
  loom_op_t* product_op = loom_scalar_defining_op(rewriter, value);
  if (!product_op || !loom_scalar_mulf_isa(product_op) ||
      !loom_scalar_fastmath_has_all(product_op,
...
static iree_status_t loom_scalar_replace_single_result_with_scaled_shift(
  ...
  if (shift < 0 || bitwidth <= 0 || shift >= bitwidth) {
    return iree_ok_status();
  }
...
static bool loom_scalar_match_addi_with_exact_constant(
  ...
  if (!add_op || !loom_scalar_addi_isa(add_op)) {
    return false;
  }
...
static bool loom_scalar_integer_value_is_low_bit_mask(int64_t value,
  ...
  *out_bit_count = 0;
  ...
  if (((unsigned_value + 1) & unsigned_value) != 0) {
    return false;
  }
  *out_bit_count =
...
iree_status_t loom_scalar_addi_canonicalize(loom_op_t* op,
  ...
  if (!loom_scalar_op_has_no_instance_flags(lhs_def)) {
    return iree_ok_status();
  }
...
iree_status_t loom_scalar_muli_canonicalize(loom_op_t* op,
  ...
  uint64_t factor = 0;
  if (loom_scalar_query_power_of_two_bits(rewriter, lhs, &factor)) {
  ...
  if (loom_scalar_query_power_of_two_bits(rewriter, rhs, &factor)) {
    return loom_scalar_replace_single_result_with_scaled_shift(
...
iree_status_t loom_scalar_negi_canonicalize(loom_op_t* op,
  ...
  if (!input_def || !loom_scalar_negi_isa(input_def)) {
    return iree_ok_status();
  }
  return loom_scalar_replace_single_result_with_value(
...
iree_status_t loom_scalar_addf_canonicalize(loom_op_t* op,
  ...
  const uint8_t add_flags = loom_scalar_addf_fastmath(op);
  if (loom_scalar_fastmath_has_all(op, LOOM_SCALAR_FASTMATHFLAGS_CONTRACT)) {
    loom_value_id_t addend = rhs;
    loom_op_t* product_op = NULL;
    if (!loom_scalar_match_contractable_mulf(rewriter, lhs, &product_op)) {
      addend = lhs;
      if (!loom_scalar_match_contractable_mulf(rewriter, rhs, &product_op)) {
        product_op = NULL;
      }
    }
    if (product_op) {
      ...
      IREE_RETURN_IF_ERROR(loom_scalar_fmaf_build(
          &rewriter->builder, fmaf_flags, loom_scalar_mulf_lhs(product_op),
          loom_scalar_mulf_rhs(product_op), addend,
          loom_scalar_single_result_type(rewriter, op), op->location,
          &fmaf_op));
      loom_value_id_t replacement = loom_scalar_fmaf_result(fmaf_op);
      IREE_RETURN_IF_ERROR(loom_rewriter_preserve_result_names_on_new_values(
          rewriter, op, &replacement, 1, value_checkpoint));
      return loom_scalar_replace_single_result_with_value(op, rewriter,
                                                          replacement);
  ...
  if (loom_scalar_value_facts_are_exact_float(rewriter, lhs, 0.0)) {
    return loom_scalar_replace_single_result_with_value(op, rewriter, rhs);
  }
  if (loom_scalar_value_facts_are_exact_float(rewriter, rhs, 0.0)) {
    return loom_scalar_replace_single_result_with_value(op, rewriter, lhs);
  }
  return iree_ok_status();
}
...
iree_status_t loom_scalar_subf_canonicalize(loom_op_t* op,
  ...
  if (loom_scalar_fastmath_has_all(op, neg_flags) &&
      loom_scalar_value_facts_are_exact_float(rewriter, lhs, 0.0)) {
    return loom_scalar_replace_single_result_with_unary_op(
...
iree_status_t loom_scalar_mulf_canonicalize(loom_op_t* op,
  ...
  if (loom_scalar_value_facts_are_exact_float(rewriter, lhs, 1.0)) {
    return loom_scalar_replace_single_result_with_value(op, rewriter, rhs);
  }
  if (loom_scalar_value_facts_are_exact_float(rewriter, rhs, 1.0)) {
    return loom_scalar_replace_single_result_with_value(op, rewriter, lhs);
  }
  ...
  if (rhs_def && loom_scalar_negf_isa(rhs_def) &&
      loom_scalar_query_exact_float(rewriter, lhs, &lhs_value)) {
    return loom_scalar_replace_mulf_with_negated_constant(
  ...
  if (loom_scalar_value_facts_are_exact_float(rewriter, lhs, 0.0) ||
      loom_scalar_value_facts_are_exact_float(rewriter, rhs, 0.0)) {
    return loom_scalar_replace_single_result_with_float_constant(op, rewriter,
                                                                 0.0);
  }
  return iree_ok_status();
}
...
iree_status_t loom_scalar_divf_canonicalize(loom_op_t* op,
  ...
  if (loom_scalar_query_exact_float(rewriter, rhs, &divisor_value)) {
...
iree_status_t loom_scalar_negf_canonicalize(loom_op_t* op,
  ...
  if (!input_def || !loom_scalar_negf_isa(input_def)) {
    return iree_ok_status();
  }
  return loom_scalar_replace_single_result_with_value(
...
iree_status_t loom_scalar_clampf_canonicalize(loom_op_t* op,
  ...
  loom_type_t result_type = loom_scalar_single_result_type(rewriter, op);
  double value = 0.0;
  if (loom_value_facts_as_exact_float(loom_type_element_type(result_type),
                                      facts, &value)) {
    return loom_scalar_replace_single_result_with_float_constant(op, rewriter,
                                                                 value);
  }
...
iree_status_t loom_scalar_fmaf_canonicalize(loom_op_t* op,
  ...
  const uint8_t zero_flags = LOOM_SCALAR_FASTMATHFLAGS_NNAN |
                             ...
                             LOOM_SCALAR_FASTMATHFLAGS_NSZ;
  if (loom_scalar_fastmath_has_all(op, zero_flags) &&
      (loom_scalar_value_facts_are_exact_float(rewriter, a, 0.0) ||
       loom_scalar_value_facts_are_exact_float(rewriter, b, 0.0))) {
    return loom_scalar_replace_single_result_with_value(op, rewriter, c);
  }
  if (loom_scalar_fastmath_has_all(op, LOOM_SCALAR_FASTMATHFLAGS_NSZ) &&
      loom_scalar_value_facts_are_exact_float(rewriter, c, 0.0)) {
    return loom_scalar_replace_single_result_with_binary_op(
        op, rewriter, LOOM_OP_SCALAR_MULF, loom_scalar_fmaf_fastmath(op), a, b);
  }
  if (loom_scalar_value_facts_are_exact_float(rewriter, a, 1.0)) {
    return loom_scalar_replace_single_result_with_binary_op(
        op, rewriter, LOOM_OP_SCALAR_ADDF, loom_scalar_fmaf_fastmath(op), b, c);
  }
  if (loom_scalar_value_facts_are_exact_float(rewriter, b, 1.0)) {
    return loom_scalar_replace_single_result_with_binary_op(
...
iree_status_t loom_scalar_powf_canonicalize(loom_op_t* op,
  ...
  if (loom_scalar_value_facts_are_exact_float(rewriter, rhs, 0.0) ||
      loom_scalar_value_facts_are_exact_float(rewriter, lhs, 1.0)) {
    return loom_scalar_replace_single_result_with_float_constant(op, rewriter,
                                                                 1.0);
  }
  if (loom_scalar_value_facts_are_exact_float(rewriter, rhs, 1.0)) {
  ...
  if (loom_scalar_value_facts_are_exact_float(rewriter, rhs, 2.0)) {
    return loom_scalar_replace_single_result_with_binary_op(

...
static iree_status_t loom_amdgpu_emit_sgpr_mul_u32(
  ...
  return loom_amdgpu_emit_binary(context, source_op,
                                 LOOM_AMDGPU_DESCRIPTOR_REF_S_MUL_I32, low_lhs,
...
iree_status_t loom_amdgpu_emit_sgpr_binary_immediate(
  ...
  *out_value = LOOM_VALUE_ID_INVALID;
  ...
  return loom_amdgpu_emit_binary(context, source_op, descriptor_ref, value,
  ...
  *out_value = LOOM_VALUE_ID_INVALID;
  ...
  if (fused_descriptor_ref != LOOM_AMDGPU_DESCRIPTOR_REF_NONE &&
      loom_amdgpu_descriptor_ref_ordinal(descriptor_set,
          ...
          LOOM_LOW_DESCRIPTOR_ORDINAL_NONE) {
    return loom_amdgpu_emit_binary(context, source_op, fused_descriptor_ref,
                                   value, addend, lane_type, out_value);
  }
  ...
  return loom_amdgpu_emit_binary(context, source_op,
                                 ...
                                 scaled_value, addend, lane_type, out_value);
}
...
iree_status_t loom_amdgpu_emit_sgpr64_add(loom_low_lower_context_t* context,
  ...
  *out_low_sum = LOOM_VALUE_ID_INVALID;
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_low_op(
      ...
      IREE_ARRAYSIZE(result_types), &low_add_lo_op));
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_low_op(
      ...
      loom_make_named_attr_slice(NULL, 0), result_types,
      IREE_ARRAYSIZE(result_types), &low_add_hi_op));

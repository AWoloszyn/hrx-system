...
#define LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_PLAN ((uint32_t)1u << 3)
...
#define LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DESCRIPTOR_MISSING \
  ((uint32_t)1u << 8)
#define LOOM_AMDGPU_ASYNC_GATHER_REJECTION_CACHE_POLICY ((uint32_t)1u << 9)
#define LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_LANE_COUNT ((uint32_t)1u << 10)
#define LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_LANE_STRIDE ((uint32_t)1u << 11)
...
static bool loom_amdgpu_async_gather_select_descriptor(
    ...
    uint32_t* out_dest_lane_byte_stride) {
  *out_descriptor_ref = LOOM_AMDGPU_DESCRIPTOR_REF_NONE;
  *out_dest_lane_byte_stride = 0;
  for (iree_host_size_t i = 0;
    ...
    *out_dest_lane_byte_stride = candidate->dest_lane_byte_stride;
    return true;
  }
  return false;
}
...
static bool loom_amdgpu_async_gather_select_source(
  ...
  if (!loom_amdgpu_async_gather_select_descriptor(
          ...
          &selection->descriptor_ref, &selection->dest_lane_byte_stride)) {
    diagnostic->rejection_bits |=
...
static bool loom_amdgpu_async_gather_select_dest(
    ...
    loom_amdgpu_async_gather_diagnostic_t* diagnostic) {
  const loom_type_t dest_type = loom_module_value_type(module, dest_view);
  const uint32_t subgroup_size = loom_amdgpu_target_wavefront_size(bundle);
  if (loom_type_dim_is_dynamic_at(dest_type, 0) ||
      loom_type_dim_static_size_at(dest_type, 0) != subgroup_size) {
    diagnostic->rejection_bits |=
  ...
  if (dest_byte_count !=
    ...
    diagnostic->rejection_bits |=
        LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_LANE_STRIDE;
    return false;
  }
...
iree_status_t loom_amdgpu_lower_kernel_async_gather(
  ...
  // The packet immediate shifts both the global and LDS addresses. Materialize
...
static iree_string_view_t loom_amdgpu_async_gather_rejection_key(
  ...
  if (iree_any_bit_set(diagnostic->rejection_bits,
                       LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_PLAN)) {
    return loom_low_source_memory_access_rejection_key(
  ...
  if (iree_any_bit_set(diagnostic->rejection_bits,
                       LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_LANE_COUNT)) {
    return IREE_SV("async_gather.dest_lane_count");
  }
  if (iree_any_bit_set(diagnostic->rejection_bits,
                       LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_LANE_STRIDE)) {
    return IREE_SV("async_gather.dest_lane_stride");
  }
  if (iree_any_bit_set(diagnostic->rejection_bits,
                       LOOM_AMDGPU_ASYNC_GATHER_REJECTION_DEST_SLOT_BASE)) {

...
iree_status_t loom_amdgpu_lower_index_cast(
  ...
  switch (plan->kind) {
    case LOOM_AMDGPU_INDEX_CAST_KIND_PRESERVING_LOW_BITS: {
      ...
      // Range refinements can retain wider storage through a value alias.
      // The cast publishes the result width selected from those refined facts.
      ...
      // A nonnegative index refinement can require a wider result than the
      // original signed index carrier. Its upper word is zero.
      if (plan->result_unit_count == 2) {
        if (loom_low_register_type_class_id(source_type) ==
...
iree_status_t loom_amdgpu_emit_sgpr64_binary_carry(
  ...
  *out_low_result = LOOM_VALUE_ID_INVALID;
  ...
  loom_value_id_t low_binary_lo_operands[] = {
      low_lhs_lo,
      low_rhs_lo,
  };
  loom_op_t* low_binary_lo_op = NULL;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_low_op(
      context, source_op, low_descriptor_ref, low_binary_lo_operands,
      IREE_ARRAYSIZE(low_binary_lo_operands),
      loom_make_named_attr_slice(NULL, 0), result_types,
      IREE_ARRAYSIZE(result_types), &low_binary_lo_op));
  const loom_value_id_t low_result_lo =
      loom_value_slice_get(loom_low_op_results(low_binary_lo_op), 0);

  loom_value_id_t low_binary_hi_operands[] = {
      low_lhs_hi,
      low_rhs_hi,
      loom_value_slice_get(loom_low_op_results(low_binary_lo_op), 1),
  };
  loom_op_t* low_binary_hi_op = NULL;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_low_op(
      context, source_op, high_descriptor_ref, low_binary_hi_operands,
      IREE_ARRAYSIZE(low_binary_hi_operands),
      loom_make_named_attr_slice(NULL, 0), result_types,
      IREE_ARRAYSIZE(result_types), &low_binary_hi_op));
  const loom_value_id_t low_result_hi =
      loom_value_slice_get(loom_low_op_results(low_binary_hi_op), 0);
...
iree_status_t loom_amdgpu_emit_i64_mul_lo(loom_low_lower_context_t* context,
  ...
  *out_low_product = LOOM_VALUE_ID_INVALID;
  ...
  const loom_amdgpu_descriptor_ref_t multiply_low =
      is_uniform ? LOOM_AMDGPU_DESCRIPTOR_REF_S_MUL_I32
                 : LOOM_AMDGPU_DESCRIPTOR_REF_V_MUL_LO_U32;
  const loom_amdgpu_descriptor_ref_t multiply_high =
      is_uniform ? LOOM_AMDGPU_DESCRIPTOR_REF_S_MUL_HI_U32
                 : LOOM_AMDGPU_DESCRIPTOR_REF_V_MUL_HI_U32;
  const loom_amdgpu_descriptor_ref_t add =
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_binary(context, source_op, multiply_low,
                                               low_lhs_lo, low_rhs_lo,
                                               lane_type, &product_lo));
  ...
  switch (plan->kind) {
    case LOOM_AMDGPU_ADDRESS_I64_ALU_KIND_SGPR_ADD: {
      ...
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_sgpr64_binary_carry(
          ...
          LOOM_AMDGPU_DESCRIPTOR_REF_S_ADDC_U32, low_lhs, low_rhs,
          &low_result));
      return loom_low_lower_bind_value(context, plan->result, low_result);
    }
    case LOOM_AMDGPU_ADDRESS_I64_ALU_KIND_VGPR_ADD: {
...
static const loom_amdgpu_descriptor_ref_t kAmdgpuScalarI64ShiftDescriptors[] = {
    ...
    [LOOM_AMDGPU_SCALAR_I64_ALU_KIND_VGPR_ASHR] =
        LOOM_AMDGPU_DESCRIPTOR_REF_V_ASHRREV_I64,
};
...
iree_status_t loom_amdgpu_lower_scalar_i64_alu(
  ...
  switch (plan->kind) {
    ...
    case LOOM_AMDGPU_SCALAR_I64_ALU_KIND_SGPR_ASHR: {
      ...
      if (plan->kind == LOOM_AMDGPU_SCALAR_I64_ALU_KIND_SGPR_ADD) {
        IREE_RETURN_IF_ERROR(loom_amdgpu_emit_sgpr64_binary_carry(
            ...
            LOOM_AMDGPU_DESCRIPTOR_REF_S_ADDC_U32, low_lhs, low_rhs,
            &low_result));
      } else if (plan->kind == LOOM_AMDGPU_SCALAR_I64_ALU_KIND_SGPR_SUB) {
        IREE_RETURN_IF_ERROR(loom_amdgpu_emit_sgpr64_binary_carry(
        ...
        const loom_module_t* module = loom_low_lower_context_module(context);
        const loom_type_t lane_type =
            loom_amdgpu_low_register_lane_type(module, low_rhs);
        loom_value_id_t low_shift;
        IREE_RETURN_IF_ERROR(loom_amdgpu_emit_low_slice(
            context, source_op, low_rhs, 0, lane_type, &low_shift));
        IREE_RETURN_IF_ERROR(loom_amdgpu_emit_binary(
            context, source_op, kAmdgpuScalarI64ShiftDescriptors[plan->kind],
            low_lhs, low_shift, loom_module_value_type(module, low_lhs),
            &low_result));
      }
      return loom_low_lower_bind_value(context, plan->result, low_result);
    }
    ...
    case LOOM_AMDGPU_SCALAR_I64_ALU_KIND_VGPR_LSHR:
    case LOOM_AMDGPU_SCALAR_I64_ALU_KIND_VGPR_ASHR: {
      loom_value_id_t low_value = LOOM_VALUE_ID_INVALID;
      IREE_RETURN_IF_ERROR(loom_amdgpu_lookup_or_materialize_vgpr_i64(

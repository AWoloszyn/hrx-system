...
static uint32_t loom_aie2p_pipeline_instance_degree(
  ...
  for (uint32_t i = 0; i < placement->plan->edge_count; ++i) {
    ...
    if (!loom_aie2p_pipeline_edge_is_internal(edge)) {
      continue;
    }
...
static bool loom_aie2p_pipeline_coordinate_compatible(
  ...
  for (uint32_t i = 0; i < placement->plan->edge_count; ++i) {
    ...
    if (other.column == UINT16_MAX || !require_adjacency) {
      continue;
    }
...
static void loom_aie2p_pipeline_candidate_affinity(
  ...
  for (uint32_t i = 0; i < placement->plan->edge_count; ++i) {
    ...
    if (column_distance == 0 && row_distance == 1) {
      ++adjacent_count;
    }
  }
...
static bool loom_aie2p_pipeline_candidate_precedes(
  ...
  if (lhs_adjacent != rhs_adjacent) {
    return lhs_adjacent > rhs_adjacent;
  }
...
static bool loom_aie2p_pipeline_place_next(
  ...
  if (placed_count == placement->plan->instance_count) {
    return true;
  }
  ...
  for (uint32_t coordinate_index = 0;
       ...
       ++coordinate_index) {
    if (coordinate_used[coordinate_index]) {
      continue;
    }
    const loom_xdna_tile_coordinate_t coordinate =
...
static bool loom_aie2p_pipeline_edges_share_source(
  ...
  if (lhs->flow_index != rhs->flow_index ||
...
static iree_status_t loom_aie2p_pipeline_emit_binding_view_endpoint(
  ...
  if (*binding_view_base == LOOM_VALUE_ID_INVALID) {
    ...
    const uint32_t endpoint_index =
        sender ? edge->source_index : edge->target_index;
    const uint32_t endpoint_port =
        sender ? edge->source_port : edge->target_port;
  ...
  return loom_aie2p_pipeline_emit_op(
      ...
      sender ? AIE2P_ARRAY_DESCRIPTOR_REF_ARRAY_PARTITION_SENDER
             : AIE2P_ARRAY_DESCRIPTOR_REF_ARRAY_PARTITION_RECEIVER,
      operands, IREE_ARRAYSIZE(operands), loom_named_attr_slice_empty(),
      &endpoint_type, location, out_endpoint);
}

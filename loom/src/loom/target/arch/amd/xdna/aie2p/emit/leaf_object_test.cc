...
TEST_F(Aie2pLeafObjectTest, LowFunctionsEmitOptimizedVectorLeaves) {
  ...
  // its last delay slot. The two loaded vectors occupy x0 and x1, preserving
  // their containing y0 aggregate. The complete native leaf occupies 30 bytes.
  struct TestCase {
...
TEST_F(Aie2pLeafObjectTest, ResourceImportsAnchorRegistersWithoutEmittingCode) {
  ...
  for (iree_host_size_t packet_index = 0;
    ...
    const loom_low_packet_view_t packet =
        loom_low_packet_at(&leaf.frame.schedule, packet_index);
    if (loom_low_resource_isa(packet.node->op)) {
      ++resource_packet_count;
    }
  }
...
TEST_F(Aie2pLeafObjectTest, DynamicFifoScanCarriesStateAcrossLoopBackedge) {
  ...
  for (iree_host_size_t packet_index = 0;
    ...
    const loom_low_packet_view_t packet =
        loom_low_packet_at(&leaf.frame.schedule, packet_index);
    if (packet.descriptor != pop_descriptor) {
      continue;
    }
    ExpectPhysicalRegisters(leaf, packet,
                            {IREE_SV("p0"), IREE_SV("lf0"), IREE_SV("r24"),

...
iree_status_t loom_bytecode_skip_type_bindings(
  ...
  iree_const_byte_span_t unused;
  return loom_bytecode_reader_read_span(decoder, cursor, length, &unused);
}
...
typedef enum loom_bytecode_attribute_value_domain_e {
  ...
  LOOM_BYTECODE_ATTRIBUTE_VALUE_DOMAIN_NONE = 2,
...
static iree_status_t loom_bytecode_attribute_retain_predicate_list(
  ...
  for (uint16_t predicate_index = 0; predicate_index < predicate_count;
    ...
    for (uint8_t i = 0; i < predicate.value.arg_count; ++i) {
      ...
      if (scope->value_domain == LOOM_BYTECODE_ATTRIBUTE_VALUE_DOMAIN_NONE) {
        return loom_bytecode_reader_emit_invalid_field(
            validator->decoder, cursor->range_name, IREE_SV("type"), 0,
            IREE_SV("predicate_value"), predicate.argument_offsets[i],
            IREE_SV("ssa_predicates_require_a_scoped_type"));
      } else if (scope->value_domain ==
                 LOOM_BYTECODE_ATTRIBUTE_VALUE_DOMAIN_NAMED) {
...
static iree_status_t loom_bytecode_attribute_validate_at_depth(
  ...
  switch (kind) {
    ...
    case LOOM_BYTECODE_ATTR_TYPE: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(validator->decoder,
                                                             cursor, &type_id));
      if (scope->value_domain == LOOM_BYTECODE_ATTRIBUTE_VALUE_DOMAIN_SSA) {
        if (type_id != 1) {
          IREE_RETURN_IF_ERROR(loom_bytecode_attribute_validate_type_ref(
              validator, type_id >> 1, available_type_count, offset));
        }
        return loom_bytecode_skip_type_bindings(validator->decoder, cursor);
      }
      IREE_RETURN_IF_ERROR(loom_bytecode_attribute_validate_type_ref(
...
iree_status_t loom_bytecode_attribute_validate_type_parameter(
    ...
    iree_host_size_t available_type_count) {
  const loom_bytecode_attribute_validation_scope_t scope = {
      .value_domain = LOOM_BYTECODE_ATTRIBUTE_VALUE_DOMAIN_NONE,
  };
  return loom_bytecode_attribute_validate_at_depth(
      validator, cursor, descriptor, kind, available_type_count, &scope, 0);
}
...
typedef loom_bytecode_attribute_materializer_t
    loom_bytecode_attribute_policy_materializer_t;

#define LOOM_BYTECODE_ATTRIBUTE_SELECTED 0
#define LOOM_BYTECODE_ATTRIBUTE_TABLES(materializer) \
  ((materializer)->module_view)
#define LOOM_BYTECODE_ATTRIBUTE_DECODE_COMPLETE_TYPE \
  loom_bytecode_attribute_decode_complete_type
#define LOOM_BYTECODE_ATTRIBUTE_DECODE_NAMED \
  loom_bytecode_attribute_decode_named
#define LOOM_BYTECODE_ATTRIBUTE_MATERIALIZE_NAMED \
  loom_bytecode_attribute_materialize_named
#define LOOM_BYTECODE_ATTRIBUTE_MATERIALIZE_SSA \
  loom_bytecode_attribute_materialize_ssa
#define LOOM_BYTECODE_TYPE_PROJECT_COMPLETED \
  loom_bytecode_type_project_completed
#define LOOM_BYTECODE_TYPE_MATERIALIZE_BINDINGS \
  loom_bytecode_type_materialize_bindings

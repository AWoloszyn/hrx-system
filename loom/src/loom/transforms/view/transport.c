...
static iree_status_t loom_view_transport_initialize_dominance(
    ...
    iree_arena_allocator_t* arena, loom_dominance_info_t* dominance) {
  *dominance = (loom_dominance_info_t){
      .module = module,
      .arena = arena,
  };
  return loom_value_fact_table_enumerate_cfg_graphs(
      facts, (loom_value_fact_cfg_graph_callback_t){
                 .user_data = dominance,
                 .fn = loom_view_transport_add_cfg_graph,
             });
}
...
static iree_status_t loom_view_transport_prepare(
  ...
  IREE_RETURN_IF_ERROR(loom_view_transport_initialize_dominance(
      plan->module, facts, plan->arena, &dominance));
  for (iree_host_size_t i = 0; i < plan->value_count; ++i) {
...
typedef struct loom_view_root_selection_source_t {
  ...
  loom_value_id_t buffer_value_id;
  // Complete or base-relative offset recipe retained by view-region analysis.
  loom_view_region_id_t region_id;
  // Selection supplying the base coordinate, or IREE_HOST_SIZE_MAX.
...
typedef struct loom_view_root_selection_plan_t {
  ...
  loom_dominance_info_t dominance;
  // Compact selection entries in physical operation order.
  ...
  // Selection indices in dependency order for coordinate materialization.
...
static bool loom_view_root_selection_plan_source(
  ...
  *out_source = (loom_view_root_selection_source_t){
  ...
  if (offset->dependency != IREE_HOST_SIZE_MAX) {
    out_source->selection_index = offset->dependency;
    return true;
  }
...
static iree_status_t loom_view_root_selection_plan_entries(
  ...
  for (iree_host_size_t i = 0; i < plan->selection_count; ++i) {
    ...
    const bool false_available = loom_view_root_selection_plan_source(
        plan, i, loom_scf_select_false_value(selection->op),
        &selection->false_source);
    selection->selected = true_available && false_available;
  }

  IREE_RETURN_IF_ERROR(
      iree_arena_allocate_array(plan->arena, plan->selection_count,
                                sizeof(*plan->order), (void**)&plan->order));
  iree_host_size_t* pending = NULL;
  IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
      plan->arena, plan->selection_count, sizeof(*pending), (void**)&pending));
  // Physical CFG block order need not follow dominance after block fusion.
  // The retained base dependencies form a DAG: cyclic transport crosses block
  // arguments or region results, which are not selection entries. Each entry
  // is stacked once and examines at most its two source dependencies.
  iree_host_size_t ordered_count = 0;
  for (iree_host_size_t i = 0; i < plan->selection_count; ++i) {
    if (plan->selections[i].ordered) {
      continue;
    }
    iree_host_size_t pending_count = 1;
    pending[0] = i;
    while (pending_count) {
      const iree_host_size_t index = pending[pending_count - 1];
      loom_view_root_selection_t* selection = &plan->selections[index];
      if (selection->selected) {
        const iree_host_size_t dependencies[] = {
            selection->true_source.selection_index,
            selection->false_source.selection_index};
        iree_host_size_t dependency = IREE_HOST_SIZE_MAX;
        for (iree_host_size_t j = 0; j < IREE_ARRAYSIZE(dependencies); ++j) {
          const iree_host_size_t source = dependencies[j];
          if (source == IREE_HOST_SIZE_MAX) {
            continue;
          }
          if (!plan->selections[source].ordered) {
            dependency = source;
            break;
          }
          selection->selected &= plan->selections[source].selected;
        }
        if (dependency != IREE_HOST_SIZE_MAX) {
          pending[pending_count++] = dependency;
          continue;
        }
      }
      selection->ordered = true;
      plan->order[ordered_count++] = index;
      --pending_count;
    }
  }
  return iree_ok_status();
...
static iree_status_t loom_view_root_selection_materialize_source(
  ...
  if (source->selection_index != IREE_HOST_SIZE_MAX) {
    ...
    *out_buffer_value_id = selection->buffer_value_id;
    plan->offsets.offsets[source->region_id].base_value_id =
        selection->offset_value_id;
  } else {
...
static iree_status_t loom_view_root_selection_rewrite(
  ...
  for (iree_host_size_t i = 0; i < plan->selection_count; ++i) {
    loom_view_root_selection_t* selection = &plan->selections[plan->order[i]];
    if (!selection->selected) {
      continue;
    }
...
static iree_status_t loom_view_root_selection_prepare(
  ...
  plan->facts = facts;
  IREE_RETURN_IF_ERROR(loom_view_transport_initialize_dominance(
      plan->module, facts, plan->arena, &plan->dominance));
  IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
  ...
  plan->offsets = (loom_view_transport_plan_t){
      ...
      .arena = plan->arena,
      .domain = plan->domain,
      .indices = plan->selection_indices,
      .regions = &regions,
  };
  IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
  ...
  IREE_RETURN_IF_ERROR(loom_view_root_selection_plan_entries(plan));

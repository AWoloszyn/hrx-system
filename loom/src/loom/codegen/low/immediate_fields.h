...
// Schema-bound fields in verified Low packet dictionaries.
...
// A fixed position in canonical dictionary order, distinct from declaration
// order in the descriptor's immediate table. Generated bindings exist only for
...
typedef uint16_t loom_low_immediate_field_t;
...
static inline loom_attribute_t loom_low_immediate_attr(
    loom_named_attr_slice_t attributes, loom_low_immediate_field_t field) {
  return attributes.entries[field].value;
}
...
static inline loom_attribute_t loom_low_optional_immediate_attr(
    loom_named_attr_slice_t attributes, loom_low_immediate_field_t field) {
  return attributes.count > field ? attributes.entries[field].value
                                  : loom_attr_absent();
}

...
class LowAllocationStorageLeaseIndexTest : public ::testing::Test {
 protected:
  ...
  std::vector<uint32_t> Query(
      ...
      loom_low_allocation_storage_lease_query_flags_t flags = 0,
    ...
    loom_low_allocation_storage_lease_unit_query_initialize(
        ...
        (uint64_t)maximum_start + 1u, flags, selection, &query);
...
TEST_F(LowAllocationStorageLeaseIndexTest, QueriesAliasingPhysicalLeaseUnits) {
  ...
  EXPECT_EQ((std::vector<uint32_t>{3}),
            Query(index, 0, UINT32_MAX, 11, 1, 1, 0, nullptr,
...
TEST_F(LowAllocationStorageLeaseIndexTest,
  ...
  EXPECT_EQ((std::vector<uint32_t>{0, 0, 1, 1, 2}),
            Query(index, 44, 44, 10, 2, 0, 0, &incoming));
  EXPECT_EQ((std::vector<uint32_t>{1, 1}),
            Query(index, 20, 20, 10, 2, 0, 0, &alternate));
  ...
  EXPECT_TRUE(Query(index, 20, 20, 10, 2, 0, 0, &incoming).empty());
}
...
TEST_F(LowAllocationStorageLeaseIndexTest, ReservesForDistinctPhysicalUnits) {
  ...
  for (uint32_t i = 0; i < kLeaseCount; ++i) {
    EXPECT_EQ((std::vector<uint32_t>{i}),
              Query(index, i * 2, i * 2, 10, 1, 0, 0, nullptr,
                    leases[i].location_kind));
  }
}

TEST_F(LowAllocationStorageLeaseIndexTest, PrunesReleasedDuplicateStartPoints) {
  ...
  EXPECT_EQ((std::vector<uint32_t>{kLeaseCount - 1}),
            Query(index, 2, 2, 10, 1, 0,
                  LOOM_LOW_ALLOCATION_STORAGE_LEASE_QUERY_FLAG_UNRELEASED));
}
...
TEST_F(LowAllocationStorageLeaseIndexTest, ReleasesLeasesDuringIteration) {
  ...
  loom_low_allocation_storage_lease_unit_query_initialize(
      &index, &descriptors_, 0, LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      10, 1, 2, 10, LOOM_LOW_ALLOCATION_STORAGE_LEASE_QUERY_FLAG_UNRELEASED,
      /*selection=*/nullptr, &query);
  uint32_t lease_index = 0;
  uint32_t count = 0;
  while (
  ...
  for (uint32_t iteration = 0; iteration < 256; ++iteration) {
    ...
    for (uint32_t i = 0; i < kLeaseCount; ++i) {
      ...
      const bool temporal =
          ...
          (flags == 0 || lease.release_action_index ==
    ...
    EXPECT_EQ(expected, Query(index, point, point, location, 3, reg_class,
                              flags, &incoming));
  }
}

...
typedef struct loom_low_schedule_resource_demand_t {
  // Sum across all simultaneous instructions before capacity admission.
  uint32_t required_units;
  // Largest reservation across all simultaneous instructions.
  uint32_t reserved_units;
} loom_low_schedule_resource_demand_t;

static loom_low_schedule_resource_demand_t
loom_low_schedule_resource_calendar_candidate_occupancy(
    ...
    uint16_t schedule_class_count, const loom_low_resource_t* resource,
  ...
  return (loom_low_schedule_resource_demand_t){
    ...
    for (uint16_t i = 0; i < schedule_class->issue_use_count; ++i) {
      ...
      for (uint32_t relative_cycle = issue_use->stage;
        ...
        const loom_low_schedule_resource_calendar_slot_t* slot =
            calendar->slots != NULL ? &calendar->slots[slot_index] : NULL;
        const loom_low_schedule_resource_occupancy_t occupied =
            slot != NULL && slot->issue_cycle == absolute_cycle
                ? slot->occupancy
...
bool loom_low_schedule_resource_group_fits(
  ...
  const loom_low_schedule_resource_calendar_t empty_calendar = {
      .descriptor_set = descriptor_set,
  };
  uint16_t bottleneck_resource_id = LOOM_LOW_RESOURCE_NONE;
  return loom_low_schedule_resource_calendar_issue_fits(
      &empty_calendar, schedule_classes, schedule_class_count, 0,
  ...
  *out_bottleneck_resource_id = LOOM_LOW_RESOURCE_NONE;
  ...
  while (true) {
    ...
    if (loom_low_schedule_resource_calendar_issue_fits(
            calendar, schedule_classes, schedule_class_count, issue_cycle,
...
iree_status_t loom_low_schedule_resource_calendar_commit(
  ...
  for (uint16_t class_index = 0; class_index < schedule_class_count;
    ...
    for (uint16_t i = 0; i < schedule_class->issue_use_count; ++i) {
      const loom_low_issue_use_t* use =
          &calendar->descriptor_set
               ->issue_uses[schedule_class->issue_use_start + i];
      maximum_end = iree_max(maximum_end, (uint32_t)use->stage + use->cycles);
    }
  }
  if (maximum_end != 0 && issue_cycle > UINT32_MAX - (maximum_end - 1u)) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "low schedule resource use exceeds the cycle "
                            "domain at issue cycle %" PRIu32,
                            issue_cycle);
  }

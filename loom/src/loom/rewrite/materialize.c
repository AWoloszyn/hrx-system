...
static iree_status_t loom_ir_clone_value_definition(
  ...
  IREE_RETURN_IF_ERROR(loom_module_define_value(
      remap->target_module, loom_type_none(), out_target_value));
  IREE_RETURN_IF_ERROR(
      loom_ir_remap_map_value(remap, source_value, *out_target_value));
  return loom_ir_clone_value_name(remap, source_value, *out_target_value);
}

static iree_status_t loom_ir_clone_block_arg_definitions(
    loom_ir_remap_t* remap, const loom_block_t* source_block,
    loom_block_t* target_block) {
  for (uint16_t i = 0; i < source_block->arg_count; ++i) {
    loom_value_id_t target_arg = LOOM_VALUE_ID_INVALID;
    IREE_RETURN_IF_ERROR(loom_ir_clone_value_definition(
...
static iree_status_t loom_ir_clone_block_arg_types(
    loom_ir_remap_t* remap, const loom_block_t* source_block,
    loom_block_t* target_block) {
  for (uint16_t i = 0; i < source_block->arg_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_module_set_value_type(
        remap->target_module, loom_block_arg_id(target_block, i), target_type));
  }
  return iree_ok_status();
}
...
static iree_status_t loom_ir_clone_op_results(loom_ir_remap_t* remap,
  ...
  for (uint16_t i = 0; i < source_op->result_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_ir_clone_value_definition(
        remap, source_results[i], &target_results[i]));
  }
  return iree_ok_status();
}
...
static iree_status_t loom_ir_clone_op_operands(
  ...
  if (loom_op_vtable_owns_operands(source_vtable)) {
...
// Parallel source and target regions owned by one recursive clone frame.
typedef struct loom_ir_clone_region_projection_t {
  // Source region whose operations are being cloned.
  const loom_region_t* source_region;
  // Target region receiving the cloned blocks.
  loom_region_t* target_region;
  // Target ordinal corresponding to source block zero.
  uint16_t target_block_index;
} loom_ir_clone_region_projection_t;
  ...
  for (uint8_t i = 0; i < source_op->successor_count; ++i) {
    ...
    if (region_projection &&
        loom_region_try_block_index(region_projection->source_region,
                                    source_successors[i], &block_index)) {
      target_successors[i] = loom_region_block(
          region_projection->target_region,
          region_projection->target_block_index + block_index);
      continue;
    }
...
static iree_status_t loom_ir_clone_region_value_definitions(
    loom_ir_remap_t* remap, const loom_region_t* source_region,
    loom_region_t* target_region, uint16_t target_block_index) {
  for (uint16_t i = 0; i < source_region->block_count; ++i) {
    const loom_block_t* source_block =
        loom_region_const_block(source_region, i);
    IREE_RETURN_IF_ERROR(loom_ir_clone_block_arg_definitions(
        remap, source_block,
        loom_region_block(target_region, target_block_index + i)));
    if (source_region->block_count > 1) {
      // CFG dominance is independent of block layout. Establish every direct
      // result identity before remapping uses or dependent block argument
      // types. Single-block regions define results as their operations are
      // cloned.
      for (const loom_op_t* op = source_block->first_op; op; op = op->next_op) {
        const loom_value_id_t* results = loom_op_const_results(op);
        for (uint16_t j = 0; j < op->result_count; ++j) {
          if (results[j] == LOOM_VALUE_ID_INVALID) {
            continue;
          }
          loom_value_id_t target_value = LOOM_VALUE_ID_INVALID;
          IREE_RETURN_IF_ERROR(
              loom_ir_clone_value_definition(remap, results[j], &target_value));
        }
      }
    }
  }
  for (uint16_t i = 0; i < source_region->block_count; ++i) {
    IREE_RETURN_IF_ERROR(loom_ir_clone_block_arg_types(
        remap, loom_region_const_block(source_region, i),
        loom_region_block(target_region, target_block_index + i)));
  }
  return iree_ok_status();
}
...
static iree_status_t loom_ir_clone_region_skeleton(
  ...
  *out_target_region = NULL;
  ...
  IREE_RETURN_IF_ERROR(loom_ir_clone_region_value_definitions(
      remap, source_region, target_region, 0));

  *out_target_region = target_region;
  return iree_ok_status();
}

// Operation and region cloning are mutually recursive through nested regions.
static iree_status_t loom_ir_clone_op_impl(
    loom_builder_t* builder, const loom_op_t* source_op, loom_ir_remap_t* remap,
    const loom_ir_clone_region_projection_t* region_projection,
    loom_op_t** out_cloned_op);

static iree_status_t loom_ir_clone_block_ops_impl(
    loom_builder_t* builder, const loom_block_t* source_block,
    loom_ir_remap_t* remap,
    const loom_ir_clone_region_projection_t* region_projection,
    const loom_ir_clone_block_options_t* options) {
  ...
  for (const loom_op_t* source_op = source_block->first_op; source_op;
    ...
    IREE_RETURN_IF_ERROR(loom_ir_clone_op_impl(builder, source_op, remap,
                                               region_projection, &cloned_op));
  }
  return iree_ok_status();
}
...
static iree_status_t loom_ir_clone_op_impl(
  ...
  *out_cloned_op = NULL;
  ...
  if (source_op->successor_count > 0) {
    ...
    IREE_RETURN_IF_ERROR(loom_ir_clone_op_successors(
        remap, region_projection, source_op, target_successors));
  }
  ...
  if (source_op->result_count > 0) {
    ...
    if (region_projection &&
        region_projection->source_region->block_count > 1) {
      const loom_value_id_t* source_results = loom_op_const_results(source_op);
      for (uint16_t i = 0; i < source_op->result_count; ++i) {
        target_results[i] = LOOM_VALUE_ID_INVALID;
        if (source_results[i] == LOOM_VALUE_ID_INVALID) {
...
iree_status_t loom_ir_clone_op(loom_builder_t* builder,
                               ...
                               loom_ir_remap_t* remap,
                               loom_op_t** out_cloned_op) {
  return loom_ir_clone_op_impl(builder, source_op, remap,
                               /*region_projection=*/NULL, out_cloned_op);
}

iree_status_t loom_ir_clone_block_ops(
    loom_builder_t* builder, const loom_block_t* source_block,
    loom_ir_remap_t* remap, const loom_ir_clone_block_options_t* options) {
  return loom_ir_clone_block_ops_impl(builder, source_block, remap,
                                      /*region_projection=*/NULL, options);
}
...
static void loom_ir_reverse_block_range(loom_block_t** blocks,
                                        uint16_t block_count) {
  for (uint16_t i = 0; i < block_count / 2; ++i) {
    loom_block_t* temporary = blocks[i];
    blocks[i] = blocks[block_count - i - 1];
    blocks[block_count - i - 1] = temporary;
  }
}

static iree_status_t loom_ir_insert_region_blocks(loom_module_t* module,
  ...
  const uint16_t old_block_count = region->block_count;
  for (uint16_t i = 0; i < block_count; ++i) {
    loom_block_t* block = NULL;
    IREE_RETURN_IF_ERROR(loom_region_append_block(module, region, &block));
  }

  // Rotate [old tail, new blocks] to [new blocks, old tail]. Appending grows
  // the table geometrically; the rotation shifts the existing tail only once.
  const uint16_t tail_block_count = old_block_count - block_index;
  loom_ir_reverse_block_range(&region->blocks[block_index], tail_block_count);
  loom_ir_reverse_block_range(&region->blocks[old_block_count], block_count);
  loom_ir_reverse_block_range(&region->blocks[block_index],
                              tail_block_count + block_count);
  for (uint16_t i = block_index; i < region->block_count; ++i) {
...
iree_status_t loom_ir_clone_region_blocks(loom_builder_t* builder,
  ...
  if (source_region->block_count > 0 && target_region->block_count > 0 &&
    ...
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "cannot insert cloned blocks before an existing region entry block");
  }
  ...
  IREE_RETURN_IF_ERROR(loom_ir_insert_region_blocks(
      builder->module, target_region, target_block_index,
  ...
  IREE_RETURN_IF_ERROR(loom_ir_clone_region_value_definitions(
      remap, source_region, target_region, target_block_index));
...
static bool loom_ir_move_op_is_inside_moved_block(
  ...
  for (const loom_op_t* current = op; current; current = current->parent_op) {
    if (current->parent_block != source_block) {
      continue;
    }
...
static bool loom_ir_move_block_arg_is_inside_moved_op(
  ...
  loom_block_for_each_op(source_block, root_op) {
    ...
    if (loom_op_contains_block(root_op, block)) {
      return true;
    }
  }
  return false;

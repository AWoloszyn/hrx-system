...
| `typed_views.cpp` | A dynamic-row/static-column source view preserves a runtime row stride and nonzero pointer origin through a by-value helper, then copies into a dense view at another interior origin. Zero and positive shapes, row padding, unchanged inputs, internal sentinels, outer guards, and device access reports are checked independently. | Original typed-view descriptor and source-lifetime witness. |

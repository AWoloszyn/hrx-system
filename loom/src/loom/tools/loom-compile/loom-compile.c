...
static void loom_compile_print_agents_markdown(FILE* stream) {
  fprintf(
      ...
      "independent,\n"
      "accept specialized config values, and leave unannotated loops alone.\n"
      "Ordinary read-ahead fits flat load/pure-compute bodies whose reads do "
      "not\n"
      "depend on carried state. Stores, nested regions, and explicit async\n"
      "groups require different ownership and are diagnosed at depth >1.\n"
      "\n"
      "```shell\n"
